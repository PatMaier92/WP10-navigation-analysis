...
plot.ms + guides(fill=F, shape=F) & theme(legend.position="right", legend.justification="top")
```
...
Looking at the memory score for well-learned items only, we found the same main effects as in the previous analysis, namely a main effect of *age group* (`r apa_lmm.ms_wl$statistic$group`, $\omega^2 =$ `r apa_post.ms_wl_omega_group`), *condition* (`r apa_lmm.ms_wl$statistic$condition`, $\omega^2 =$ `r apa_post.ms_wl_omega_condition`) and *session* (`r apa_lmm.ms_wl$statistic$session`, $\omega^2 =$ `r apa_post.ms_wl_omega_session`). In addition, we did find evidence for an *age group x session* interaction (`r apa_lmm.ms_wl$statistic$group_session`, $\omega^2 =$ `r apa_post.ms_wl_omega_group_session`) for well-learned items. Post-hoc contrasts revealed there were no initial age-related differences in the memory score for well-learned items during learning (session 1 for 6-8-yo vs. 9-11-yo: `r apa_post.ms_wl_group_session$statistic$X1_YoungKids_OldKids`; 6-8-yo vs. adults: `r apa_post.ms_wl_group_session$statistic$X1_YoungKids_YoungAdults`; 9-11-yo vs. adults: `r apa_post.ms_wl_group_session$statistic$X1_OldKids_YoungAdults`) but that adults performed significantly better than both 6-8-yo children (`r apa_post.ms_wl_group_session$statistic$X2_YoungKids_YoungAdults`) and 9-11-yo children ((`r apa_post.ms_wl_group_session$statistic$X2_OldKids_YoungAdults`) after the two-week delay period. This implies that adults may profit from superior consolidation compared to children, considering a comparable level of initial learning. 

The LMM had a significantly better fit when including the *random slope for session* compared to a model with random intercept only ($\chi^2$(`r apa_rs.session$Df`) = `r apa_rs.session$Chisq`, p `r apa_rs.session$p`), meaning that there was still notable interindividual variation in participant's forgetting rates across the two-week delay period. 
...
### Supplement: Seperate models with well-learned items
