...
apa_post.latency_group <- post.latency_group %>%  apa_print() %>% bonferroni_fix()

# condition
apa_post.latency_omega_condition <- omega.latency %>% 
  filter(Parameter=="condition") %>% select(Omega2_partial) %>% apa_num()

apa_post.latency_condition <- post.latency_condition %>%  apa_print() %>% bonferroni_fix()

# random effects LRT 
apa_rs.condition <- LRT.latency %>% mutate_at(vars(Chisq, Df), apa_df) %>% mutate_at(vars(p), apa_p, add_equals=T)


rm(post.latency_group_condition, post.latency_group, post.latency_condition, LRT.latency, omega.latency)
```
...
```{r, include=F}
rm(model.latency, R2, apa_lmm.latency, apa_post.latency_group_condition, apa_post.latency_group, apa_post.latency_condition, apa_post.latency_omega_group_condition, apa_post.latency_omega_group, apa_post.latency_omega_condition, apa_rs.condition)
```
