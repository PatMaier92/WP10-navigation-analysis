...
```{r, echo=F, fig.height=7, fig.width=10, fig.cap="Memory accuracy and navigation efficiency in learning session" }
plot.ms + guide_area() + plot_spacer() + plot_spacer() +
  plot.latency + plot.path + plot.distance + plot.rotation_velocity  +
  plot_annotation(tag_levels=list(c("A", "B", NULL, NULL, NULL))) + 
  plot_layout(nrow=2, guides="collect") & guides(fill=F, shape=F) & theme(legend.direction="vertical")

#rm(plot.ms, plot.latency, plot.path, plot.distance, plot.rotation_velocity)
```
...
### Egocentric immediate memory  

```{r, model_plsc_ego_l, include=F}
```

```{r, plot_plsc_scatter_ego_l, include=F}
```

```{r, plot_plsc_lv_ego_l, include=F}
```

```{r apa_plsc_ego_l, include=F}

# LV 
apa_lv.weights_ego_l <- plsc_ego_lv_l %>% 
  slice(1) %>% 
  mutate_all(as.numeric) %>% 
  mutate_at(vars(-LV), apa_num) %>% mutate_at(vars(LV), apa_p, add_equals=T)

apa_lv.cor_ego_l <- plsc_ego_lv_l %>% slice(2) %>% apa_num()
  
# LPS 
apa_aov.plsc_ego_l <- apa_print(model.plsc_ego_l)

apa_post.plsc_ego_l <- post.plsc_ego_l %>% apa_print() %>% bonferroni_fix()

apa_cor_r.plsc_ego_l <- cor.plsc_ego_l$r %>% mutate_all(apa_num)
apa_cor_p.plsc_ego_l <- cor.plsc_ego_l$p %>% mutate_all(apa_p, add_equals=T)


rm(plsc_ego_lv_l, model.plsc_ego_l, post.plsc_ego_l, cor.plsc_ego_l)
```

We applied PLSC to identify unique profiles in relation to either egocentric or allocentric immediate memory across age groups. For the egocentric condition, we identified a single reliable LV ($p$ `r apa_lv.weights_ego_l$LV`) that optimally represents associations between  egocentric memory and a profile of navigational indicators from the learning session and post-navigational indicators ($r$ = `r apa_lv.cor_ego_l$LV`). We identified several stable components within the multivariate profile. Better immediate egocentric memory was associated with a specific navigation pattern, more specifically with shorter latency (BSR = `r apa_lv.weights_ego_l$latency`, $r$ = `r apa_lv.cor_ego_l$latency`), more direct navigation (excess path length; BSR = `r apa_lv.weights_ego_l$excess_path`, $r$ = `r apa_lv.cor_ego_l$excess_path`), and search near the goal (excess distance to goal; BSR = `r apa_lv.weights_ego_l$excess_distance`, $r$ = `r apa_lv.cor_ego_l$excess_distance`) but not with visual exploration at the first decision point (initial rotation velocity; BSR = `r apa_lv.weights_ego_l$rotation_velocity`, $r$ = `r apa_lv.cor_ego_l$rotation_velocity`). Better immediate egocentric memory was also associated with the post-navigational tests, more specifically with a higher layout score (BSR = `r apa_lv.weights_ego_l$layout`, $r$ = `r apa_lv.cor_ego_l$layout`) and landmark positioning score (BSR = `r apa_lv.weights_ego_l$position`, $r$ = `r apa_lv.cor_ego_l$position`) but not with the landmark identity score (BSR = `r apa_lv.weights_ego_l$landmark`, $r$ = `r apa_lv.cor_ego_l$landmark`) . 

As a second step, we investigated age-related differences in the egocentric latent profile score (E-LPS). A higher E-LPS indicates that an individual expressed the latent profile more strongly, which was associated with better egocentric immediate memory. One-way ANOVA revealed significant age group differences in E-LPS (`r apa_aov.plsc_ego_l$statistic`), indicating that there was a gradual increase in the expression of the E-LPS with increasing age (6-8-yo vs. 9-11-yo: `r apa_post.plsc_ego_l$statistic$YoungKids_OldKids`; 6-8-yo vs. adults: `r apa_post.plsc_ego_l$statistic$YoungKids_YoungAdults`; 9-11-yo vs adults: `r apa_post.plsc_ego_l$statistic$OldKids_YoungAdults`). The correlation coefficients between egocentric immediate memory and E-LPS did differ significantly between age groups (6-8-yo $r$ = `r apa_cor_r.plsc_ego_l$"6-8-yo"`, 9-11-yo: $r$ = `r apa_cor_r.plsc_ego_l$"9-11-yo"`, adults: $r$ = `r apa_cor_r.plsc_ego_l$"adults"`; 6-8-yo vs. 9-11-yo: $p$ `r apa_cor_p.plsc_ego_l$"6-8-yo vs. 9-11-yo"`; 6-8-yo vs. adults: $p$ `r apa_cor_p.plsc_ego_l$"6-8-yo vs. adults"`; 9-11-yo vs. adults: $p$ `r apa_cor_p.plsc_ego_l$"9-11-yo vs. adults"`).

```{r, include=F}
rm(apa_lv.weights_ego_l, apa_aov.plsc_ego_l, apa_post.plsc_ego_l, apa_lv.cor_ego_l, apa_cor_r.plsc_ego_l, apa_cor_p.plsc_ego_l)
```

### Allocentric immediate memory  

```{r, model_plsc_allo_l, include=F}
```

```{r, plot_plsc_scatter_allo_l, include=F}
```

```{r, plot_plsc_lv_allo_l, include=F}
```

```{r apa_plsc_allo_l, include=F}

# LV 
apa_lv.weights_allo_l <- plsc_allo_lv_l %>% 
  slice(1) %>% 
  mutate_all(as.numeric) %>% 
  mutate_at(vars(-LV), apa_num) %>% mutate_at(vars(LV), apa_p, add_equals=T)

apa_lv.cor_allo_l <- plsc_allo_lv_l %>% slice(2) %>% apa_num()

# LPS
apa_aov.plsc_allo_l <- apa_print(model.plsc_allo_l)

apa_post.plsc_allo_l <- post.plsc_allo_l %>% apa_print() %>% bonferroni_fix()

apa_cor_r.plsc_allo_l <- cor.plsc_allo_l$r %>% mutate_all(apa_num)
apa_cor_p.plsc_allo_l <- cor.plsc_allo_l$p %>% mutate_all(apa_p, add_equals=T)


rm(plsc_allo_lv_l, model.plsc_allo_l, post.plsc_allo_l, cor.plsc_allo_l)
```

For the allocentric condition, we identified a single reliable LV ($p$ `r apa_lv.weights_allo_l$LV`) that optimally represents associations between immediate allocentric memory and a profile of navigational indicators from the learning session and post-navigational indicators ($r$ = `r apa_lv.cor_allo_l$LV`). We identified several stable components within the multivariate profile. Better immediate allocentric memory was associated with a specific navigation pattern during learning, more specifically with shorter latency (BSR = `r apa_lv.weights_allo_l$latency`, $r$ = `r apa_lv.cor_allo_l$latency`), more direct navigation (excess path length; BSR = `r apa_lv.weights_allo_l$excess_path`, $r$ = `r apa_lv.cor_allo_l$latency`), search near the goal (excess distance to goal; BSR = `r apa_lv.weights_allo_l$excess_distance`, $r$ = `r apa_lv.cor_allo_l$excess_path`), and more visual exploration at the first decision point (initial rotation velocity; BSR = `r apa_lv.weights_allo_l$rotation_velocity`, $r$ = `r apa_lv.cor_allo_l$rotation_velocity`). Better immediate allocentric memory was also associated with the post-navigational tests, more specifically with a higher layout score (BSR = `r apa_lv.weights_allo_l$layout`, $r$ = `r apa_lv.cor_allo_l$layout`), landmark identity score (BSR = `r apa_lv.weights_allo_l$landmark`, $r$ = `r apa_lv.cor_allo_l$landmark`) and landmark positioning score (BSR = `r apa_lv.weights_allo_l$position`, $r$ = `r apa_lv.cor_allo_l$position`). 

As a second step, we investigated age-related differences in the allocentric latent profile score (A-LPS). A higher A-LPS indicates that an individual expressed the latent profile more strongly, which was associated with better allocentric immediate memory. One-way ANOVA revealed significant age group differences in A-LPS (`r apa_aov.plsc_allo_l$statistic`), indicating that there was a gradual increase in the expression of the A-LPS with increasing age (6-8-yo vs. 9-11-yo: `r apa_post.plsc_allo_l$statistic$YoungKids_OldKids`; 6-8-yo vs. adults: `r apa_post.plsc_allo_l$statistic$YoungKids_YoungAdults`; 9-11-yo vs adults: `r apa_post.plsc_allo_l$statistic$OldKids_YoungAdults`). The correlation coefficients between allocentric long-delay memory and A-LPS did not differ between age groups (6-8-yo $r$ = `r apa_cor_r.plsc_allo_l$"6-8-yo"`, 9-11-yo: $r$ = `r apa_cor_r.plsc_allo_l$"9-11-yo"`, adults: $r$ = `r apa_cor_r.plsc_allo_l$"adults"`; 6-8-yo vs. 9-11-yo: $p$ `r apa_cor_p.plsc_allo_l$"6-8-yo vs. 9-11-yo"`; 6-8-yo vs. adults: $p$ `r apa_cor_p.plsc_allo_l$"6-8-yo vs. adults"`; 9-11-yo vs. adults: $p$ `r apa_cor_p.plsc_allo_l$"9-11-yo vs. adults"`).

```{r, include=F}
rm(apa_lv.weights_allo_l, apa_aov.plsc_allo_l, apa_post.plsc_allo_l, apa_cor_r.plsc_allo_l, apa_cor_p.plsc_allo_l)
```

```{r, echo=F, fig.height=7.8, fig.width=7.8, fig.cap="PLSC multivariate profiles" }

plot.plsc_lv_ego_l + 
  (plot.plsc_ego_l + theme(axis.title.x=element_text(margin=margin(t=-150, unit="pt")),
                         legend.direction="horizontal", legend.position=c(0,-0.5))) + 
  plot.plsc_ego_lps_l + 
  plot.plsc_lv_allo_l + 
  (plot.plsc_allo_l + theme(axis.title.x=element_text(margin=margin(t=-150, unit="pt")),
                          legend.direction="horizontal", legend.position=c(0,-0.5))) + 
  plot.plsc_allo_lps_l + 
  plot_annotation(tag_levels=list(c("A", "", "", "B", "", ""))) + 
  plot_layout(nrow=2, widths=c(0.325, 0.5, 0.175)) 

rm(plot.plsc_lv_ego_l, plot.plsc_ego_l, plot.plsc_ego_lps_l, plot.plsc_lv_allo_l, plot.plsc_allo_l, plot.plsc_allo_lps_l)
```
