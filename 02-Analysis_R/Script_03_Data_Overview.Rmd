...
apa_emm1 <- emmeans(model.ms$full_model, pairwise ~ group, 
                    lmer.df="satterthwaite", adjust="bonferroni")$contrasts %>% 
  apa_print() %>% bonferroni_fix()

apa_emm1b <- summary(emmeans(model.ms, ~ group, lmer.df="satterthwaite"),
                     null=0.5, adjust="bonferroni", infer=c(T,T)) %>% 
    apa_print() %>% bonferroni_fix()

apa_emm2 <- emmeans(model.ms$full_model, pairwise ~ condition, lmer.df="satterthwaite")$contrasts %>% apa_print()

apa_emm3 <- emmeans(model.ms$full_model, pairwise ~ session, lmer.df="satterthwaite")$contrasts %>% apa_print()
```
...
We found significant main effects of *age group* (`r apa_lmm$statistic$group`), *condition* (`r apa_lmm$statistic$condition`), and *session* (`r apa_lmm$statistic$session`) for the memory score in probe trials, but no significant two-way or three-way interactions (Table\ \@ref(tab:table-probe-ms-f0)). All age groups performed significantly above chance level (6-7-yo: `r apa_emm1b$statistic$YoungKids`; 9-10-yo: `r apa_emm1b$statistic$OldKids`; adults: `r apa_emm1b$statistic$YoungAdults`) and there was a gradual increase in memory with increasing age (`r apa_emm1$statistic$YoungKids_OldKids`; `r apa_emm1$statistic$YoungKids_YoungAdults`; `r apa_emm1$statistic$OldKids_YoungAdults`). The memory score was slightly higher in the egocentric than the allocentric condition (`r apa_emm2$full_result`) and spatial memory deteriorated significantly over the 2-week period between the sessions (`r apa_emm3$full_result`). 
...
```{r, include=F}
rm(model.ms, R2, apa_lmm, apa_emm1, apa_emm1b, apa_emm2, apa_emm3)
```
...
apa_p.layout <- model.layout %>% pluck("p.value") %>% apa_p(add_equals=T)
apa_post_p.layout_young_vs_old <- post.layout$p.adj[1] %>% apa_p(add_equals=T)
apa_post_p.layout_young_vs_adult <- post.layout$p.adj[2] %>% apa_p(add_equals=T)
apa_post_p.layout_old_vs_adult <- post.layout$p.adj[3] %>% apa_p(add_equals=T)
```

We found significant *age-related group* differences in the ability to correctly identify the maze layout (Fisher's exact test $p$`r apa_p.layout`). Post-hoc contrasts showed that all age groups differed significantly from each other and that there was a gradual increase in performance with increasing age (Bonferroni-adjusted pairwise Fisher test: 6-7-yo vs. 9-10-yo $p$ `r apa_post_p.layout_young_vs_old`; 6-7-yo vs. adults $p$ `r apa_post_p.layout_young_vs_adult`; 9-10-yo vs. adults $p$ `r apa_post_p.layout_old_vs_adult`).

```{r, include=F}
rm(model.layout, apa_p.layout, post.layout, apa_post_p.layout_young_vs_old, apa_post_p.layout_young_vs_adult, apa_post_p.layout_old_vs_adult)
```
...
apa_post_p.position <- post.position$contrasts %>% apa_print() %>% bonferroni_fix()
```

There were significant *age-related group* differences in the ability to correctly position the landmarks and goal locations in a 2D map (`r apa_aov.position$statistic$group`). Post-hoc contrasts showed that adults performed significantly better than both groups of children (6-7-yo vs. adults `r apa_post_p.position$statistic$YoungKids_YoungAdults`; 9-10-yo vs. adults `r apa_post_p.position$statistic$OldKids_YoungAdults`) but that the children groups did not differ (6-7-yo vs. 9-10-yo `r apa_post_p.position$statistic$YoungKids_OldKids`). 

```{r, include=F}
rm(model.position, apa_aov.position, apa_post_p.position)
```
...
apa_lmm <- apa_print(model.rotation)
apa_lmm <- apa_lmm$table 
...
* *group x session x condition* interaction: 
...
```{r, include=F}
rm(model.rotation, apa_lmm, apa_emm4)
```
...
apa_lmm <- apa_print(model.rotation_learn)
apa_lmm <- apa_lmm$table 
...
The model had a fit of $R2 = `r R2$Rsq`, [CI: `r R2$upper.CL`, `r R2$lower.CL`$].
(Table\ \@ref(tab:table-learn-rot-p0))

ref:l-irp-caption0) Fixed effects for initial rotation in training.
...
```{r, include=F}
rm(model.rotation_learn, R2, apa_lmm)
```
