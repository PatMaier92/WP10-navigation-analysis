...
We first computed LMMs for latency, excess path length, excess distance to goal and initial rotation in training trials and tested for changes across trials (learning curves) and age differences (Table 2; Figure 2A). 
...
Similarly, participants in all age groups had a significant decrease in excess path length across trials. When comparing the age groups, we found that adults navigated more directly (i.e., shorter excess path length) than 6-8YO children (`r apa_post.train_path_group$statistic$YoungKids_YoungAdults`) and 9-11YO children (`r apa_post.train_path_group$statistic$OldKids_YoungAdults`). 9-11YO children navigated more directly than 6-8YO children (`r apa_post.train_path_group$statistic$YoungKids_OldKids`). 
...
In summary, participants from all age groups were able to significantly improve their navigation behavior across training trials (decrease in latency, excess path length and excess distance to goal). Additionally, older participants had a more efficient search and navigation behavior, which was characterized by quicker navigation (latency), more direct navigation (excess path length), more focused search near the goal location (excess distance to goal) and more visual exploration and deliberation at the beginning of the training phase (initial rotation). Navigation differences were pronounced between 6-8YO children in comparison with both 9-10YO children and adults, but less evident when comparing 9-10YO children to adults. An additional analysis of navigation behavior in probe trials showed similar results (details in supplement). 
...
Next, we wanted to know how well participants from different age groups were able to form egocentric and allocentric spatial memories during the learning session. We computed a LMM with the memory score and tested for condition (egocentric vs. allocentric) and age differences (Table 3; Figure 2B). 

Adults had better immediate spatial memory than 9-11YO children (`r apa_post.ms_group$statistic$OldKids_YoungAdults`) and 6-8YO children (`r apa_post.ms_group$statistic$YoungKids_YoungAdults`). 9-11YO children had better immediate spatial memory than 6-8YO children (`r apa_post.ms_group$statistic$YoungKids_OldKids`). Participants in all age groups reached above chance-level performance (tests against 0.5 all p < .001, details in supplement). Participants had slightly better egocentric than allocentric memory (Table 3). There was also a trend for an interaction between condition and age group, indicating that this advantage for egocentric memory was present in 9-11YO children (`r apa_post.ms_group_condition$statistic$OldKids_Egoret_alloret`) but not in 6-8YO children (`r apa_post.ms_group_condition$statistic$YoungKids_Egoret_alloret`) or adults (`r apa_post.ms_group_condition$statistic$YoungAdults_Egoret_alloret`). The LMM with memory score had a significantly better fit when including the random slope for condition ($\chi^2$(`r apa_rs.condition$Df`) = `r apa_rs.condition$Chisq`, p `r apa_rs.condition$p`), meaning that there was substantial interindividual variation in how participants responded to the two conditions.

In summary, we found pronounced age differences in memory performance during the learning session. With increasing age, participants were able to form more precise spatial memories in both the egocentric and the allocentric condition. 

SUPPLEMENT: Participants in all age groups reached above chance-level performance for the memory score in the learning session. Tests against 0.5 for 
6-8YO children: `r apa_post.ms_group_chance$full_result$YoungKids`, 9-11YO children: `r apa_post.ms_group_chance$full_result$OldKids`, and adults: `r apa_post.ms_group_chance$full_result$YoungAdults`. 
...
In summary we found no evidence for an improved memory consolidation in adults compared to children. Age differences in long-delay memory after two weeks were largely driven by age differences in learning.
...
After two weeks, we also tested if participants were able to correctly identify the maze boundary, identify the landmark objects, and position the landmarks in a cognitive map (Figure 3B). 

We found significant age differences in the ability to identify the maze boundary (Fisher's exact test $p$ `r apa_fisher.layout`). Adults were better than 9-11YO children ($p$ `r apa_post.layout_old_vs_adult`) and 6-8YO children ($p$ `r apa_post.layout_young_vs_adult`), and 9-11YO children outperformed 6-8YO children ($p$ `r apa_post.layout_young_vs_old`). 6-8YO children were not better than chance (`r apa_post.layout_change_young$statistic`) and chose randomly between the options (pairwise Fisher's exact test: no $p$ < .05). In contrast, almost half of the 9-11YO children (`r apa_post.layout_change_old$statistic`) and almost all of the adults (`r apa_post.layout_change_adult$statistic`) selected the correct layout. 
...
Lastly, we did find significant age differences in the ability to correctly position the landmark and goal objects in a map (`r apa_aov.position$statistic$group`). Here, adults performed significantly better than 6-8YO children (`r apa_post.position$statistic$YoungKids_YoungAdults`) and 9-11YO children (`r apa_post.position$statistic$OldKids_YoungAdults`) but the children groups did not differ from each other (`r apa_post.position$statistic$YoungKids_OldKids`). 

In summary, adults had a superior spatial knowledge at the end of the experiment, both regarding the geometrical structure of the maze and the landmark positioning. On the other hand, both groups of children were as proficient as adults in identifying perceptually detailed environmental landmarks. 
...
Finally, we wanted to test if the age-related variations in (a) navigation behavior during the initial training phase and (b) spatial knowledge that emerged after training can predict spatial memory. We applied PLSC and identified a single reliable LV ($p$ `r apa_lv.p`; Figure 4A) that optimally represents associations between age and a profile of navigation indicators from training trials in the learning session (`r apa_model.age_lp_n$r_and_p`) and spatial knowledge indicators from the post-navigational task (`r apa_model.age_lp_p$r_and_p`). We identified several stable components within the multivariate profile (BSR; values ±1.96 are significant α < .05), paralleling our previous results. Higher age was associated with a specific navigation pattern during the initial training phase i.e., shorter latency (BSR = `r apa_bsr$latency`), more direct navigation (excess path length; BSR = `r apa_bsr$excess_path`), and search near the goal (excess distance to goal; BSR = `r apa_bsr$excess_distance `) but not visual exploration at the first decision point (initial rotation; BSR = `r apa_bsr$initial_rotation `). Higher age was also associated with specific pattern of spatial knowledge at the end of the experiment i.e., a higher boundary score (BSR = `r apa_bsr$layout `) and landmark positioning score (BSR = `r apa_bsr$position`) but not landmark identity score (BSR = `r apa_bsr$landmark `). 

As a second step, we computed two latent profile scores (LPS) for each participant, one for navigation and one for spatial knowledge. A more positive LPS indicates that an individual expressed the age-related profile more strongly, which means that children with a high LPS score expressed more adult-like behavior. We then correlated the LPS with immediate and long-delay egocentric and allocentric memory scores (Table 5; Figure 4B/4C). The navigation LPS correlated significantly with immediate egocentric (`r apa_model.lp_n_ego_1$r_and_p`) and long-delay egocentric memory (`r apa_model.lp_n_ego_2$r_and_p`), and the former was significantly higher (`r p_bonferroni4` `r apa_model.p_lp_n_ego`). The navigation LPS also correlated significantly with immediate allocentric (`r apa_model.lp_n_allo_1$r_and_p`) and long-delay allocentric memory (`r apa_model.lp_n_allo_2$r_and_p`). The correlation with immediate egocentric memory was significantly higher than with immediate allocentric memory (`r p_bonferroni4` `r apa_model.p_lp_n_ego_allo_1`). In conclusion, we found that navigation behavior during training predicted later memory performance in the egocentric and allocentric probe conditions, both short-term and long-term. Participants with a more efficient, adult-like navigation during training had better spatial memory in probe trials. There was a particularly strong association with immediate egocentric memory. The spatial knowledge LPS also correlated significantly with immediate egocentric (`r apa_model.lp_p_ego_1$r_and_p`), long-delay egocentric (`r apa_model.lp_p_ego_2$r_and_p`), immediate allocentric (`r apa_model.lp_p_allo_1$r_and_p`) and long-delay allocentric memory (`r apa_model.lp_p_allo_2$r_and_p`). Thus, participants with a more mature understanding of the maze boundary and landmark positioning had better spatial memory in the egocentric and allocentric probe conditions, both short-term and long-term.
...
```{r, include=F}
ggsave("../WP10_data/Plots/bsr_learn.jpg", plot.bsr_learn, width=4.8, height=3.2, dpi=600)
...
ggsave("../WP10_data/Plots/age_x_lp_post_learn.jpg", plot.age_x_lp_post_learn, width=2, height=3, dpi=600)

ggsave("../WP10_data/Plots/mem_x_lp_nav_learn.jpg", plot.mem_x_lp_nav_learn, width=3, height=3, dpi=600)

ggsave("../WP10_data/Plots/mem_x_lp_post_learn.jpg", plot.mem_x_lp_post_learn, width=3, height=3, dpi=600)
...
In summary, we found pronounced age differences in navigation behavior during the learning session probe trials, paralleling the results for training trials. Older participants showed more efficient behavior which was characterized by quicker navigation (shorter latency), more direct navigation (shorter excess path length) and more focused search near the goal location (lower excess distance to goal). In addition, navigation was quicker, more direct, focused more on the goal location and involved less initial rotation in the egocentric compared to the allocentric condition. 
