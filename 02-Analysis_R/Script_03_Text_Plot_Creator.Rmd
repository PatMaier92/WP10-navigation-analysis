...
In summary, we found pronounced age differences in memory performance during the learning session. Older participants were able to form more precise spatial memories in the egocentric and the allocentric condition. 
...
We found significant age differences in the ability to correctly identify the geometrical maze layout (Fisher's exact test $p$ `r apa_fisher.layout`) with a gradual performance increase with increasing age (6-8YO vs. 9-11YO: $p$ `r apa_post.layout_young_vs_old`; 6-8YO vs. AD: $p$ `r apa_post.layout_young_vs_adult`; 9-11YO vs. AD: $p$ `r apa_post.layout_old_vs_adult`). 6-8YO children were not better than chance (`r apa_post.layout_change_young$statistic`) and chose randomly between the options (pairwise Fisher's exact test: no $p$ < .05). In contrast, almost half of the 9-11YO children (`r apa_post.layout_change_old$statistic`) and almost all of the adults (`r apa_post.layout_change_adult$statistic`) selected the correct layout. 
...
In summary, adults had a superior spatial knowledge at the end of the experiment, both regarding the geometrical structure of the maze and the object-to-object relations in the environment. On the other hand, both groups of children were as proficient as adults when simply identifying environmental landmarks. 
