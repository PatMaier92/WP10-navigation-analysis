...
apa_model.p_lp_n_ego <- model.comp_lp_n_ego$p %>% as.numeric() %>% apa_p(add_equals=T)

apa_model.lp_n_allo_1 <- model.lp_n_allo_1 %>% apa_print()
apa_model.lp_n_allo_2 <- model.lp_n_allo_2 %>% apa_print()
apa_model.p_lp_n_allo <- model.comp_lp_n_allo$p %>% as.numeric() %>% apa_p(add_equals=T)

rm(lv.p, lv.cor, lv.ulcor, lv.llcor, model.lp_n_ego_1, model.lp_n_ego_2, model.lp_n_allo_1, model.lp_n_allo_2, model.comp_lp_n_ego, model.comp_lp_n_allo)
```
...
As a second step, we computed the latent profile score (LPS) for each participant. A higher LPS indicates that an individual expressed the age-related profile more strongly, meaning that children with a higher LPS navigate more similar to adults. We then correlated the LPS with the immediate and long-delay memory scores for the egocentric and the allocentric condition. LPS correlated significantly with immediate egocentric (`r apa_model.lp_n_ego_1$full_result`) and long-delay egocentric memory (`r apa_model.lp_n_ego_2$full_result`). The correlation with immediate egocentric memory was significantly higher (p `r apa_model.p_lp_n_ego`). LPS correlated significantly with immediate allocentric (`r apa_model.lp_n_allo_1$full_result`) and long-delay allocentric memory (`r apa_model.lp_n_allo_2$full_result`). The correlations with immediate and long-delay allocentric memory were equally strong (p `r apa_model.p_lp_n_allo`). In conclusion, we found that participants with a more efficient, adult-like navigation behavior in the initial training phase of the learning session had better immediate and long-delay spatial memory.
...
```{r include=F}
rm(apa_lv.p, apa_lv.cor, apa_lv.ulcor, apa_lv.llcor, apa_bsr, apa_model.lp_n_ego_1, apa_model.lp_n_ego_2, apa_model.lp_n_allo_1, apa_model.lp_n_allo_2, apa_model.p_lp_n_ego, apa_model.p_lp_n_allo)
```
...
rm(plot.bsr_learn, plot.age_x_lp_nav_learn, plot.age_x_lp_post_learn)
```
