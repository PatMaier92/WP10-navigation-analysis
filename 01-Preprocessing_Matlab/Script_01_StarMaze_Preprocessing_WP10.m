...
%             sm.participant(p).session(s).trial(k).support.new_sampling_rate=sum(sampling_rate_new)/length(sampling_rate_new);
            ...
            %% time analysis
            sm.participant(p).session(s).trial(k).time=computeTime(t(1),t(end)); % total amount of time
            ...
            if sm.participant(p).session(s).trial(k).condition~=4
                %% compute variables depending on this trial's settings
...
%                 end
                ...
                [sm.participant(p).session(s).trial(k).chosen_goal_i, ...
                    ...
                    sm.participant(p).session(s).trial(k).obj_at_chosen_loc]=sm_wp10_chosenGoal(...
...
%                         sm.participant(p).session(s).trial(k).support.ideal_avg_distance_ego_target);
                ...
                [sm.participant(p).session(s).trial(k).zone.alley_zone,...
                    ...
                    sm.participant(p).session(s).trial(k).zone.alley_rotations]=sm_wp10_coordinatesZonesStatic(x,...
                ...
                [sm.participant(p).session(s).trial(k).zone.alley_zone_out,...
                    ...
                    sm.participant(p).session(s).trial(k).zone.alley_rotations_out]=sm_wp10_coordinatesZonesStatic(x,...
                ...
                [sm.participant(p).session(s).trial(k).zone.alley_zone_in,...
                    ...
                    sm.participant(p).session(s).trial(k).zone.alley_rotations_in]=sm_wp10_coordinatesZonesStatic(x,...
                ...
                [sm.participant(p).session(s).trial(k).zone.pentagon_zone,...
                    ...
                    sm.participant(p).session(s).trial(k).zone.pentagon_rotations]=sm_wp10_coordinatesPentagon(x,...
                ...
                [sm.participant(p).session(s).trial(k).zone.triangle_zone,....
                    ...
                    sm.participant(p).session(s).trial(k).zone.triangle_rotations]=sm_wp10_coordinatesZonesStatic(x,...
                ...
                [sm.participant(p).session(s).trial(k).zone.rectangle_zone,...
                    ...
                    sm.participant(p).session(s).trial(k).zone.rectangle_rotations]=sm_wp10_coordinatesZonesStatic(x,...
                ...
                [sm.participant(p).session(s).trial(k).zone.alley_time, ...
                    ...
                    sm.participant(p).session(s).trial(k).zone.rectangle_time]=sm_wp10_timeInZone(...
                    sm.participant(p).session(s).trial(k).time, ...
                    sm.participant(p).session(s).trial(k).zone.rel_alley_zone, ...
                    sm.participant(p).session(s).trial(k).zone.rel_pentagon_zone, ...
                    sm.participant(p).session(s).trial(k).zone.rel_triangle_zone, ...
                ...
                sm.participant(p).session(s).trial(k).arm_explored=sm_wp10_armExplored(...
                    sm.participant(p).session(s).trial(k).zone.alley_zone);
                sm.participant(p).session(s).trial(k).arm_score=sm_wp10_armScore(...
                    sm.participant(p).session(s).trial(k).zone.alley_entry);
                ...
                [sm.participant(p).session(s).trial(k).success, ...
                    sm.participant(p).session(s).trial(k).success_ego,...
                    sm.participant(p).session(s).trial(k).correct_final_alley,...
                    sm.participant(p).session(s).trial(k).correct_final_alley_ego]=sm_wp10_success(...
                    success_criterium,...
                    sm.participant(p).session(s).trial(k).final_distance,...
                    sm.participant(p).session(s).trial(k).final_distance_ego,...
                    sm.participant(p).session(s).trial(k).goal_alley,...
                    sm.participant(p).session(s).trial(k).chosen_alley_int,...
...
%                     sm.participant(p).session(s).trial(k).path_score);
                ...
                if sm.participant(p).session(s).trial(k).chosen_alley_int==999
                ...
                elseif sm.participant(p).session(s).trial(k).condition ~=4 % not for motor control task
                    if (sm.participant(p).session(s).trial(k).path_length<=0.1 ...
                            || sm.participant(p).session(s).trial(k).sum_body_rotation==0 ...
                ...
                % Path analysis
                sm.participant(p).session(s).trial(k).path_length=0; % reset/initiate variables
                for i=1:sdata_length
                    % PATH to all TARGETS
                    % cumulative distance traveled (used in path accuracy)
                    sm.participant(p).session(s).trial(k).path_length=sm.participant(p).session(s).trial(k).path_length+sum(computeDistance(x(i),x(i+1),y(i),y(i+1)));
                end
                              
                % PATH ACCURACY to all TARGETS
                sm.participant(p).session(s).trial(k).path_accuracy=computeDeviationToIdealValue(sm.participant(p).session(s).trial(k).path_length,sm.participant(p).session(s).trial(k).support.ideal_path_length);
