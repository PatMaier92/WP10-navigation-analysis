...
%                 end
                ...
                % Arm score and Path score as indicators of exploration
                sm.participant(p).session(s).trial(k).arm_explored=sm_wp10_armExplored(...
                    sm.participant(p).session(s).trial(k).zone.alley_zone);
                sm.participant(p).session(s).trial(k).arm_score=sum(sm.participant(p).session(s).trial(k).zone.alley_entry);
