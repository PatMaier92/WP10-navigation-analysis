...
%             sm.subject(partNo).session(sesNo).trial(k).support.newAvgSamplingRate=sum(newSamplingRate)/length(newSamplingRate);
            ...
            if sm.subject(partNo).session(sesNo).trial(k).trial_condition==4
                ...
                [sm.subject(partNo).session(sesNo).trial(k).support.goal_x_ego, ...
                    sm.subject(partNo).session(sesNo).trial(k).support.goal_y_ego, ...
                    x_line, y_line, x_line_chosen, y_line_chosen, x_line_ego, y_line_ego, ...
                    sm.subject(partNo).session(sesNo).trial(k).support.ideal_path_length, ...
                    sm.subject(partNo).session(sesNo).trial(k).support.ideal_path_length_chosen, ...
                    sm.subject(partNo).session(sesNo).trial(k).support.ideal_path_length_ego, ...
                    sm.subject(partNo).session(sesNo).trial(k).support.ego_alley, ...
                    sm.subject(partNo).session(sesNo).trial(k).support.ideal_headturnNo]=sm_wp10_depStartVariables(...
                    sm.coord.myGraph, sm.coord.graph_x, sm.coord.graph_y, ...
                    sm.subject(partNo).session(sesNo).trial(k).start_int, ...
                    sm.subject(partNo).session(sesNo).trial(k).goal_int,...
...
%                     hold off
                ...
                [~, ~, sm.subject(partNo).session(sesNo).trial(k).support.zone.ideal_alley_entry, ...
                    ...
                    yi_al, zeros(length(xi_al),1), alley_full_x, alley_full_y);
                
                [~, ~, sm.subject(partNo).session(sesNo).trial(k).support.zone.ideal_rectangle_entry, ...
                    ~]= sm_wp10_coordinatesZonesStatic(xi_al,...
                    yi_al, zeros(length(xi_al),1), rec_x, rec_y);
                
%                 [ideal_alley_entry_mat]=sm_wp10_coordinatesZonesDynamic(xi_al,...
%                     yi_al, alley_full_x, alley_full_y);
...
%                 if mod(sm.subject(partNo).session(sesNo).trial(k).start_int,2) % even number = outer start
...
%                 end
                ...
                for i=1:sdata_length
                    % PATH to target
                    % cumulative distance traveled (used in path accuracy)
                    sm.subject(partNo).session(sesNo).trial(k).result.path_length=sm.subject(partNo).session(sesNo).trial(k).result.path_length+sm_distance(x(i),x(i+1),y(i),y(i+1));
%                     % DISTANCE to CORRECT target
%                     % cumulative distance to target (used in distance analysis)
%                     total_dist_to_goal=total_dist_to_goal+sm_distance(x(i),sm.subject(partNo).session(sesNo).trial(k).goal_x,y(i),sm.subject(partNo).session(sesNo).trial(k).goal_y);
%                     % DISTANCE to EGOCENTRIC target
%                     % cumulative distance to egocentric target (used in distance analysis)
%                     total_dist_to_goal_ego=total_dist_to_goal_ego+sm_distance(x(i),sm.subject(partNo).session(sesNo).trial(k).support.goal_x_ego,y(i),sm.subject(partNo).session(sesNo).trial(k).support.goal_y_ego);
...
%                     total_dist_to_chosen_goal=total_dist_to_chosen_goal+sm_distance(x(i),x(end),y(i),y(end));
                ...
                sm.subject(partNo).session(sesNo).trial(k).result.final_distance=0;
                if sm.subject(partNo).session(sesNo).trial(k).feedback==0
                    sm.subject(partNo).session(sesNo).trial(k).result.final_distance=sm_distance(...
                        sm.subject(partNo).session(sesNo).trial(k).goal_x, ...
                        sm.subject(partNo).session(sesNo).trial(k).result.x_end, ...
                        sm.subject(partNo).session(sesNo).trial(k).goal_y, ...
                        sm.subject(partNo).session(sesNo).trial(k).result.y_end);
                end
                ...
                distance_to_path = [distance_to_path; sm.subject(partNo).session(sesNo).trial(k).result.final_distance]; % add final distance as last data point
                sm.subject(partNo).session(sesNo).trial(k).result.avg_distance_path=mean(distance_to_path);
...
%                 sm.subject(partNo).session(sesNo).trial(k).result.total_distance_target=total_dist_to_goal;
...
%                 sm.subject(partNo).session(sesNo).trial(k).support.ideal_avg_distance_target=id_total_dist_to_goal/xi_length;
%                 sm.subject(partNo).session(sesNo).trial(k).support.ideal_total_distance_target=id_total_dist_to_goal;
%                 sm.subject(partNo).session(sesNo).trial(k).support.ideal_sum_data_points=xi_length;
...
%                     sm.subject(partNo).session(sesNo).trial(k).result.avg_distance_target, ...
%                     sm.subject(partNo).session(sesNo).trial(k).support.ideal_avg_distance_target);
                
                % VELOCITY
                sm.subject(partNo).session(sesNo).trial(k).result.velocity=sm.subject(partNo).session(sesNo).trial(k).result.path_length / ...
                    sm.subject(partNo).session(sesNo).trial(k).result.time;
...
%                     sm.subject(partNo).session(sesNo).trial(k).result.distance_accuracy_chosen=sm_ac(sm.subject(partNo).session(sesNo).trial(k).result.avg_distance_chosen_target,sm.subject(partNo).session(sesNo).trial(k).support.ideal_avg_distance_chosen_target);
                ...
                if sm.subject(partNo).session(sesNo).trial(k).trial_condition==1 && ...
                        mod(sm.subject(partNo).session(sesNo).trial(k).start_int,2)
                    ...
                    sm.subject(partNo).session(sesNo).trial(k).result.final_distance_ego=sm_distance(...
                        sm.subject(partNo).session(sesNo).trial(k).support.goal_x_ego,...
                        sm.subject(partNo).session(sesNo).trial(k).result.x_end,...
                        sm.subject(partNo).session(sesNo).trial(k).support.goal_y_ego,...
                        sm.subject(partNo).session(sesNo).trial(k).result.y_end);
                    ...
                    distance_to_ego_path = [distance_to_ego_path; sm.subject(partNo).session(sesNo).trial(k).result.final_distance_ego]; % add final distance as last data point
                    sm.subject(partNo).session(sesNo).trial(k).result.avg_distance_ego_path=mean(distance_to_ego_path);
                    sm.subject(partNo).session(sesNo).trial(k).result.total_distance_ego_path=sum(distance_to_ego_path);
                    ...
                    distance_to_ego_path_unique = [distance_to_ego_path_unique; sm.subject(partNo).session(sesNo).trial(k).result.final_distance_ego]; % add final distance as last data point
                    sm.subject(partNo).session(sesNo).trial(k).result.avg_distance_ego_path_pure=mean(distance_to_ego_path_unique);
                    sm.subject(partNo).session(sesNo).trial(k).result.total_distance_ego_path_pure=sum(distance_to_ego_path_unique);
                    
%                     % AVERAGE DISTANCE to EGOCENTRIC target
%                     sm.subject(partNo).session(sesNo).trial(k).result.avg_distance_ego_target=total_dist_to_goal_ego/sdata_length(1);
%                     sm.subject(partNo).session(sesNo).trial(k).result.total_distance_ego_target=total_dist_to_goal_ego;
%                     % sum data points: same as in analyis above
...
%                     sm.subject(partNo).session(sesNo).trial(k).support.ideal_avg_distance_ego_target=id_total_dist_to_goal_ego/xi_eg_length;
%                     sm.subject(partNo).session(sesNo).trial(k).support.ideal_total_distance_ego_target=id_total_dist_to_goal_ego;
%                     sm.subject(partNo).session(sesNo).trial(k).support.ideal_sum_data_points_ego=xi_eg_length;
...
%                         sm.subject(partNo).session(sesNo).trial(k).result.avg_distance_ego_target, ...
%                         sm.subject(partNo).session(sesNo).trial(k).support.ideal_avg_distance_ego_target);
                else
                    % set dummy values
                    sm.subject(partNo).session(sesNo).trial(k).result.final_distance_ego=999;
                    sm.subject(partNo).session(sesNo).trial(k).result.avg_distance_ego_path=999;
                    sm.subject(partNo).session(sesNo).trial(k).result.total_distance_ego_path=999;
                    sm.subject(partNo).session(sesNo).trial(k).result.avg_distance_ego_path_pure=999;
                    sm.subject(partNo).session(sesNo).trial(k).result.total_distance_ego_path_pure=999;
                    sm.subject(partNo).session(sesNo).trial(k).result.avg_distance_ego_target=999;
                    sm.subject(partNo).session(sesNo).trial(k).result.total_distance_ego_target=999;
                    sm.subject(partNo).session(sesNo).trial(k).result.path_accuracy_ego=999;
...
%                 end
%                 sm.subject(partNo).session(sesNo).trial(k).result.sum_body_rotation=sum(br);
%                 
%                 % Ideal sum of body roatations
%                 l_xi_al=length(xi_al);
%                 sm.subject(partNo).session(sesNo).trial(k).support.ideal_sum_body_rotation=0; br_i=zeros(1,l_xi_al);
%                 for i=2:(l_xi_al-2)
%                     br_i(i)=sm_b_rot(yi_al(i-1),xi_al(i-1),yi_al(i),xi_al(i));
%                 end
%                 sm.subject(partNo).session(sesNo).trial(k).support.ideal_sum_body_rotation=sum(br_i);
%                 
%                 % Body-rotation-accuracy
%                 sm.subject(partNo).session(sesNo).trial(k).result.body_rotation_accuracy=sm_ac(sm.subject(partNo).session(sesNo).trial(k).result.sum_body_rotation,sm.subject(partNo).session(sesNo).trial(k).support.ideal_sum_body_rotation);
%                 
...
%                 sm.subject(partNo).session(sesNo).trial(k).result.body_turn_left=0;sm.subject(partNo).session(sesNo).trial(k).result.body_turn_right=0;body_walk_straight=0;
%                 for j=2:(length(br)-1)
%                     body_turn(j)=heaviside((br(j+1)-br(j)));
%                     if body_turn(j)==1 && (body_turn(j-1)==0 || body_turn(j-1)==0.5)
%                         sm.subject(partNo).session(sesNo).trial(k).result.body_turn_right=sm.subject(partNo).session(sesNo).trial(k).result.body_turn_right+1;
%                     elseif body_turn(j)==0 &&(body_turn(j-1)==1 || body_turn(j-1)==0.5)
%                         sm.subject(partNo).session(sesNo).trial(k).result.body_turn_left=sm.subject(partNo).session(sesNo).trial(k).result.body_turn_left+1;
%                     else
%                         body_walk_straight=body_walk_straight+1;
%                     end
%                 end
%                 sm.subject(partNo).session(sesNo).trial(k).result.no_body_turn=sm.subject(partNo).session(sesNo).trial(k).result.body_turn_right+sm.subject(partNo).session(sesNo).trial(k).result.body_turn_left;
%                 
%                 % Cumulative ideal body turns
%                 body_turn_i=zeros(1,l_xi_al);
%                 sm.subject(partNo).session(sesNo).trial(k).support.ideal_body_turn_left=0;sm.subject(partNo).session(sesNo).trial(k).support.ideal_body_turn_right=0;ideal_body_walk_straight=0;
%                 for j=2:(length(br_i)-1)
%                     body_turn_i(j)=heaviside((br_i(j+1)-br_i(j)));
%                     if body_turn_i(j)==1 && (body_turn_i(j-1)==0 || body_turn_i(j-1)==0.5)
%                         sm.subject(partNo).session(sesNo).trial(k).support.ideal_body_turn_right=sm.subject(partNo).session(sesNo).trial(k).support.ideal_body_turn_right+1;
%                     elseif body_turn_i(j)==0 &&(body_turn_i(j-1)==1 || body_turn_i(j-1)==0.5)
%                         sm.subject(partNo).session(sesNo).trial(k).support.ideal_body_turn_left=sm.subject(partNo).session(sesNo).trial(k).support.ideal_body_turn_left+1;
%                     else
%                         ideal_body_walk_straight=ideal_body_walk_straight+1;
%                     end
%                 end
%                 sm.subject(partNo).session(sesNo).trial(k).support.ideal_no_body_turn=sm.subject(partNo).session(sesNo).trial(k).support.ideal_body_turn_right+sm.subject(partNo).session(sesNo).trial(k).support.ideal_body_turn_left;
%                 
%                 % Body-turn-accuracy
%                 sm.subject(partNo).session(sesNo).trial(k).result.body_turn_accuracy=sm_ac(sm.subject(partNo).session(sesNo).trial(k).result.no_body_turn,sm.subject(partNo).session(sesNo).trial(k).support.ideal_no_body_turn);
%                 
...
%                 
%                 ro=length(r); sm.subject(partNo).session(sesNo).trial(k).result.final_deviation=0; % final deviation from start to target angle
%                 sm.subject(partNo).session(sesNo).trial(k).result.sum_head_rotation=0; % cumulative rotation, sum of head rotations
%                 for j=1:(ro-1)
%                     sm.subject(partNo).session(sesNo).trial(k).result.final_deviation=sm.subject(partNo).session(sesNo).trial(k).result.final_deviation+(r(j+1)-r(j)); % deviation
%                     sm.subject(partNo).session(sesNo).trial(k).result.sum_head_rotation=sm.subject(partNo).session(sesNo).trial(k).result.sum_head_rotation+abs(r(j+1)-r(j)); % sum of head roations
%                 end
%                 sm.subject(partNo).session(sesNo).trial(k).result.no_full_head_rotation=sm.subject(partNo).session(sesNo).trial(k).result.sum_head_rotation/360;
%                 
...
%                 sm.subject(partNo).session(sesNo).trial(k).result.head_turn_left=0;sm.subject(partNo).session(sesNo).trial(k).result.head_turn_right=0;head_walk_straight=0;head_left=0; head_right=0;
%                 for j=2:(ro-1)
%                     head_turn(j)=heaviside((r(j+1)-r(j)));
%                     if head_turn(j)==1 && (head_turn(j-1)==0 || head_turn(j-1)==0.5)
%                         sm.subject(partNo).session(sesNo).trial(k).result.head_turn_right=sm.subject(partNo).session(sesNo).trial(k).result.head_turn_right+1;
%                     elseif head_turn(j)==0 && (head_turn(j-1)==1 || head_turn(j-1)==0.5)
%                         sm.subject(partNo).session(sesNo).trial(k).result.head_turn_left=sm.subject(partNo).session(sesNo).trial(k).result.head_turn_left+1;
...
%                 
%                 % Head-turn total
%                 sm.subject(partNo).session(sesNo).trial(k).result.no_head_turn=sm.subject(partNo).session(sesNo).trial(k).result.head_turn_right+sm.subject(partNo).session(sesNo).trial(k).result.head_turn_left;
%                 
%                 % Head-turn-accuracy
%                 sm.subject(partNo).session(sesNo).trial(k).result.head_turn_accuracy=sm_ac(sm.subject(partNo).session(sesNo).trial(k).result.no_head_turn,sm.subject(partNo).session(sesNo).trial(k).support.ideal_headturnNo);
%                 
%                 fprintf('Head turn analysis done for %d, session %d, file no %d.\n', subj, sesNo, k);
                
                %% Zone analysis
                [sm.subject(partNo).session(sesNo).trial(k).result.zone.alley_zone,...
                    sm.subject(partNo).session(sesNo).trial(k).result.zone.rel_alley_zone,...
                    sm.subject(partNo).session(sesNo).trial(k).result.zone.alley_entry,...
                    sm.subject(partNo).session(sesNo).trial(k).result.zone.alley_rotations]=sm_wp10_coordinatesZonesStatic(x,...
                    y,r,alley_full_x,alley_full_y);
                
                [alley_entry_mat]=sm_wp10_coordinatesZonesDynamic(x,...
                    y,alley_full_x,alley_full_y);
                
                [sm.subject(partNo).session(sesNo).trial(k).result.zone.alley_zone_out,...
                    sm.subject(partNo).session(sesNo).trial(k).result.zone.rel_alley_zone_out,...
                    sm.subject(partNo).session(sesNo).trial(k).result.zone.alley_entry_out,...
                    sm.subject(partNo).session(sesNo).trial(k).result.zone.alley_rotations_out]=sm_wp10_coordinatesZonesStatic(x,...
                    y,r,alley_half_out_x,alley_half_out_y);
                
                [sm.subject(partNo).session(sesNo).trial(k).result.zone.alley_zone_in,...
                    sm.subject(partNo).session(sesNo).trial(k).result.zone.rel_alley_zone_in,...
                    sm.subject(partNo).session(sesNo).trial(k).result.zone.alley_entry_in,...
                    sm.subject(partNo).session(sesNo).trial(k).result.zone.alley_rotations_in]=sm_wp10_coordinatesZonesStatic(x,...
                    y,r,alley_half_in_x,alley_half_in_y);
                
                [sm.subject(partNo).session(sesNo).trial(k).result.zone.pentagon_zone,...
                    sm.subject(partNo).session(sesNo).trial(k).result.zone.rel_pentagon_zone,...
                    sm.subject(partNo).session(sesNo).trial(k).result.zone.pentagon_entry,...
                    sm.subject(partNo).session(sesNo).trial(k).result.zone.pentagon_rotations]=sm_wp10_coordinatesPentagon(x,...
                    y,r,cP_x,cP_y);
                
                [sm.subject(partNo).session(sesNo).trial(k).result.zone.triangle_zone,....
                    sm.subject(partNo).session(sesNo).trial(k).result.zone.rel_triangle_zone,...
                    sm.subject(partNo).session(sesNo).trial(k).result.zone.triangle_entry,...
                    sm.subject(partNo).session(sesNo).trial(k).result.zone.triangle_rotations]=sm_wp10_coordinatesZonesStatic(x,...
                    y,r,tri_x,tri_y);
                
                [sm.subject(partNo).session(sesNo).trial(k).result.zone.rectangle_zone,...
                    sm.subject(partNo).session(sesNo).trial(k).result.zone.rel_rectangle_zone,...
                    sm.subject(partNo).session(sesNo).trial(k).result.zone.rectangle_entry,...
                    sm.subject(partNo).session(sesNo).trial(k).result.zone.rectangle_rotations]=sm_wp10_coordinatesZonesStatic(x,...
                    y,r,rec_x,rec_y);
                
                [rectangle_entry_mat]=sm_wp10_coordinatesZonesDynamic(x,...
                    y,rec_x,rec_y);
                
                [sm.subject(partNo).session(sesNo).trial(k).result.zone.alley_time, ...
                    sm.subject(partNo).session(sesNo).trial(k).result.zone.pentagon_time, ...
                    sm.subject(partNo).session(sesNo).trial(k).result.zone.triangle_time, ...
                    sm.subject(partNo).session(sesNo).trial(k).result.zone.rectangle_time]=sm_wp10_timeInZone(...
                    sm.subject(partNo).session(sesNo).trial(k).result.time, ...
                    sm.subject(partNo).session(sesNo).trial(k).result.zone.rel_alley_zone, ...
                    sm.subject(partNo).session(sesNo).trial(k).result.zone.rel_pentagon_zone, ...
                    sm.subject(partNo).session(sesNo).trial(k).result.zone.rel_triangle_zone, ...
                ...
                sm.subject(partNo).session(sesNo).trial(k).result.arm_explored=sm_wp10_armExplored(...
                    sm.subject(partNo).session(sesNo).trial(k).result.zone.alley_zone);
                sm.subject(partNo).session(sesNo).trial(k).result.arm_score=sm_wp10_armScore(...
                    sm.subject(partNo).session(sesNo).trial(k).result.zone.alley_entry);
                sm.subject(partNo).session(sesNo).trial(k).result.path_explored=sm_wp10_pathExplored(...
                    sm.subject(partNo).session(sesNo).trial(k).result.zone.alley_zone_out,...
                    sm.subject(partNo).session(sesNo).trial(k).result.zone.alley_zone_in,...
                    sm.subject(partNo).session(sesNo).trial(k).result.zone.rectangle_zone,...
                    sm.subject(partNo).session(sesNo).trial(k).result.zone.triangle_zone);
                sm.subject(partNo).session(sesNo).trial(k).result.path_score=sm_wp10_pathScore(...
                    sm.subject(partNo).session(sesNo).trial(k).result.zone.alley_entry_out,...
                    sm.subject(partNo).session(sesNo).trial(k).result.zone.alley_entry_in,...
                    sm.subject(partNo).session(sesNo).trial(k).result.zone.rectangle_entry,...
                    sm.subject(partNo).session(sesNo).trial(k).result.zone.triangle_entry);
                ...
                [sm.subject(partNo).session(sesNo).trial(k).result.success, ...
                    sm.subject(partNo).session(sesNo).trial(k).result.success_ego,...
                    sm.subject(partNo).session(sesNo).trial(k).result.correct_final_alley,...
                    sm.subject(partNo).session(sesNo).trial(k).result.correct_final_alley_ego]=sm_wp10_success(...
                    success_criterium,...
                    sm.subject(partNo).session(sesNo).trial(k).result.final_distance,...
                    sm.subject(partNo).session(sesNo).trial(k).result.final_distance_ego,...
                    sm.subject(partNo).session(sesNo).trial(k).goal_alley,...
                    sm.subject(partNo).session(sesNo).trial(k).result.chosen_alley_int,...
                    sm.subject(partNo).session(sesNo).trial(k).support.ego_alley);
                
%                 % Direct path to target
%                 [ideal_no, match_abs, ...
%                     sm.subject(partNo).session(sesNo).trial(k).result.direct_path_percent, ...
%                     sm.subject(partNo).session(sesNo).trial(k).result.direct_path]=sm_wp10_directPathCalc(...
%                     uniq_alley, uniq_rect, alley_entry_mat, rectangle_entry_mat);
                
%                 % Search strategies
%                 [sm.subject(partNo).session(sesNo).trial(k).result.search_strategy_direct,...
%                     sm.subject(partNo).session(sesNo).trial(k).result.search_strategy_detour,...
%                     sm.subject(partNo).session(sesNo).trial(k).result.search_strategy_reoriented,...
%                     sm.subject(partNo).session(sesNo).trial(k).result.search_strategy_no]=sm_wp10_searchStrategy(...
%                     sm.subject(partNo).session(sesNo).trial(k).result.direct_path,...
%                     sm.subject(partNo).session(sesNo).trial(k).result.path_explored,...
%                     sm.subject(partNo).session(sesNo).trial(k).result.path_score);
                ...
                sm.subject(partNo).session(sesNo).trial(k).exclude_trial_matlab=0;
                if sm.subject(partNo).session(sesNo).trial(k).result.chosen_alley_int==999
                    sm.subject(partNo).session(sesNo).trial(k).exclude_trial_matlab=1;
                    fprintf('Trial %d marked for exclusion due to timeout.\n',k);
                elseif sm.subject(partNo).session(sesNo).trial(k).trial_condition ~=4 % not for motor control task
                    if (sm.subject(partNo).session(sesNo).trial(k).result.path_length<=0.1 ...
                            || sm.subject(partNo).session(sesNo).trial(k).result.sum_body_rotation==0 ...
                            || sm.subject(partNo).session(sesNo).trial(k).result.time < 3)
                        sm.subject(partNo).session(sesNo).trial(k).exclude_trial_matlab=1;
            ...
            %     'rotation_tri_1', 'rotation_tri_2', 'rotation_tri_3', 'rotation_tri_4', 'rotation_tri_5',...
            ...
            %% Create plots
            if sesNo==3
                sm_wp10_plotMotorControl(sm.subject(partNo).session(sesNo).trial(k).trial_num,...
                    sm.subject(partNo).session(sesNo).session_number,...
                    sm.subject(partNo).id,sm.subject(partNo).Group,...
                    pm.coord.pract_polyshape,- pm.coord.goal_x, pm.coord.goal_y, ...
                    pm.coord.start_x, pm.coord.start_y,...
                    pm.coord.pract_goal_locs, x, y ,xi_al, yi_al, folderOut)
            else
                sm_wp10_plotTrack(sm.subject(partNo).session(sesNo).trial(k).trial_num, ...
                    sm.subject(partNo).session(sesNo).session_number, ...
                    sm.subject(partNo).session(sesNo).trial(k).trial_condition, ...
                    sm.subject(partNo).session(sesNo).trial(k).start_int, ...
                    sm.subject(partNo).id,sm.subject(partNo).Group, ...
                    sm.subject(partNo).session(sesNo).trial(k).result.success, ...
                    sm.subject(partNo).session(sesNo).trial(k).result.direct_path, ...
                    sm.subject(partNo).session(sesNo).trial(k).result.search_strategy_no, ...
                    alley_polyshape_1, alley_polyshape_2, sm.coord.tri, sm.coord.rec, ...
