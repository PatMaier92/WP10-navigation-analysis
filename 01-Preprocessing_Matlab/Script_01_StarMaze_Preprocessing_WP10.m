...
%             sm.participant(p).session(s).trial(k).new_sampling_rate=sum(sampling_rate_new)/length(sampling_rate_new);
            ...
            if sm.participant(p).session(s).trial(k).condition~=4
                ...
                % TBD: x_line_chosen/y_line_chosen are wrong! 
...
%                 end
                ...
                % compute short zone sequence (10 zones) for ACTUAL path 
                % code: alleys (ACEGI), rectangles (BDFHI)
                ...
                % compute short zone sequence (10 zones) for IDEAL path to
                % target
                ...
                % TBD use for strategy comparison: compute short zone sequence (10 zones) for IDEAL path to
                % chosen goal 
                [ideal_seq_10_ch]=computeZoneSequence(x_line_chosen, y_line_chosen, sm.coord.alley_full_x, sm.coord.alley_full_y,...
                    sm.coord.alley_half_out_x, sm.coord.alley_half_out_y,...
                    sm.coord.alley_half_in_x, sm.coord.alley_half_in_y,...
                    sm.coord.rec_x, sm.coord.rec_y, sm.coord.tri_x, sm.coord.tri_y, 10);

                % compute long zone sequence (based on 20 zones) for ACTUAL path
                % code: outer alleys (ACEGI), inner alleys (acegi),
                ...
                % EDIT DISTANCE 
                % compute edit distance cost (method: Levenshtein) 
                % this function calculates deviation between ideal and actual path sequence 
                sm.participant(p).session(s).trial(k).zone_editdistance=editDistance(seq_10, ideal_seq_10);
                
                % SHORTEST PATH to target
                % defined as edit distance of zero (no deviation)
                sm.participant(p).session(s).trial(k).direct_path=sm.participant(p).session(s).trial(k).zone_editdistance==0; 
                
                % TBD: Update SEARCH STRATEGY CATEGORY
                [sm.participant(p).session(s).trial(k).search_strategy_direct,...
                    ...
                    sm.participant(p).session(s).trial(k).zones_explored,...
                    sm.participant(p).session(s).trial(k).zones_entered);
                clear *seq*; 
