...
%             save(fullfile(output_folder, file_name_Subject),'sm', '-append');
            ...
            %     'trial','block','trial_in_block','condition',...
            ...
            %     sm.sub{p}.session{s}.trial{k}.trial_in_block sm.sub{p}.session{s}.trial{k}.condition ...
            %     sm.sub{p}.session{s}.trial{k}.start sm.sub{p}.session{s}.trial{k}.goal_i ...
            %     sm.sub{p}.session{s}.trial{k}.goal_alley sm.sub{p}.session{s}.trial{k}.goal_identity ...
            %     sm.sub{p}.session{s}.trial{k}.feedback sm.sub{p}.session{s}.trial{k}.ego_alley ...
            %     sm.sub{p}.session{s}.trial{k}.result.chosen_goal_i sm.sub{p}.session{s}.trial{k}.result.chosen_alley_int ...
            %     sm.sub{p}.session{s}.trial{k}.result.obj_at_chosen_loc sm.sub{p}.session{s}.trial{k}.exclude_trial_matlab ];
            % result_folder = ['results_' sm.sub{p}.group_s '_' num2str(i_participant) '_' Session '_' Trial '.xls'];
            % new_file = fullfile(output_folder, result_folder);
            %
            % % write data
    ...
    % new_name2 = [sm.sub{p}.group_s '_' num2str(sm.sub{p}.id)  '_results'];
    % new_file = fullfile(result_folder, new_name2);
    % sm_wp10_table_allTrials(output_folder,new_file,col_header,col_header_2,col_header_3,'BU','EO','data_vars','support_vars');
