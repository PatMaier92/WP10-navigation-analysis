...
% read-in data 
M = readmatrix(fullfile(folderIn, name));
...
sm.sub{partNo}.session{sesNo}.trial{k}.x_start=x(1,1); sm.sub{partNo}.session{sesNo}.trial{k}.y_start=y(1,1);
sm.sub{partNo}.session{sesNo}.trial{k}.x_end=x(end,1); sm.sub{partNo}.session{sesNo}.trial{k}.y_end=y(end,1);
      
%% Get single trial info from trial_results.csv
sm.sub{partNo}.session{sesNo}.session_duration=round(minutes(trial_data.timestamp(numel(files),1) - trial_data.timestamp(1,1))); % provide session duration

sm.sub{partNo}.session{sesNo}.trial{k}.block=trial_data.block_num(k,1);
sm.sub{partNo}.session{sesNo}.trial{k}.trial_num=trial_data.trial_num(k,1);
sm.sub{partNo}.session{sesNo}.trial{k}.trial_in_block=trial_data.trial_num_in_block(k,1);     

sm.sub{partNo}.session{sesNo}.trial{k}.feedback=trial_data.trial_feedback(k,1);
[sm.sub{partNo}.session{sesNo}.trial{k}.fb]=sm_wp10_feedback(sm.sub{partNo}.session{sesNo}.trial{k}.feedback);

sm.sub{partNo}.session{sesNo}.trial{k}.trial_type=trial_data.trial_type(k,1);
sm.sub{partNo}.session{sesNo}.trial{k}.trial_condition=sm_wp10_trialCondition(sm.sub{partNo}.session{sesNo}.trial{k}.trial_type,sm.sub{partNo}.session{sesNo}.trial{k}.fb);

n_goals=4; 
sm.sub{partNo}.session{sesNo}.trial{k}.trial_goal_identity=sm_wp10_trialGoalIdentity(n_goals, char(trial_data.trial_goal_identity(k,1)));

sm.sub{partNo}.session{sesNo}.trial{k}.trial_goal=trial_data.trial_goal(k,1);
[sm.sub{partNo}.session{sesNo}.trial{k}.goal_x,sm.sub{partNo}.session{sesNo}.trial{k}.goal_y,...
    sm.sub{partNo}.session{sesNo}.trial{k}.goal_int,sm.sub{partNo}.session{sesNo}.trial{k}.goal_str,...
    sm.sub{partNo}.session{sesNo}.trial{k}.goal_alley]=sm_wp10_trialGoal(char(sm.sub{partNo}.session{sesNo}.trial{k}.trial_goal),...
    sm.coord.goal_x,sm.coord.goal_y,sm.coord.goal_locs,sm.coord.alley_locs);

sm.sub{partNo}.session{sesNo}.trial{k}.trial_startpos=trial_data.trial_player(k,1);
[sm.sub{partNo}.session{sesNo}.trial{k}.start]=sm_wp10_trialStart(sm.sub{partNo}.session{sesNo}.trial{k}.trial_startpos,sm.coord.start_locs);

%% Time analysis using timestamp
b=t(end); a=t(1);
sm.sub{partNo}.session{sesNo}.trial{k}.result.time=sm_time(a,b); % total amount of time
     
fprintf('Time analysis done for %d, session %d, file no %d.\n', subj, session, k);
    
%% For motor control navigation trial 
% analysis of path and time 
if sm.sub{partNo}.session{sesNo}.trial{k}.trial_condition==4
    ...
    sm.sub{partNo}.session{sesNo}.trial{k}.ideal_path_length=sm_wp10_idealPathLength(x_line_motor, y_line_motor);
    ...
    [xi_al,yi_al]=sm_wp10_dataInterpolation(x_line_motor, y_line_motor,...
        sm.sub{partNo}.session{sesNo}.trial{k}.ideal_path_length); 
    
    %% Time analysis: already done 
    %% Coordinate analysis using x-/y-coordinates
    % Path analysis
    sm.sub{partNo}.session{sesNo}.trial{k}.result.path_length=0; % reset/initiate variables
    for i=1:sdata_length
        % PATH to all TARGETS
        % cumulative distance traveled (used in path accuracy)
        sm.sub{partNo}.session{sesNo}.trial{k}.result.path_length=sm.sub{partNo}.session{sesNo}.trial{k}.result.path_length+sum(sm_distance(x(i),x(i+1),y(i),y(i+1)));
    end
    
    % Interpolated IDEAL PATH LENGTH value
    sm.sub{partNo}.session{sesNo}.trial{k}.ideal_path_length_interpol=0; % start-initiation
    xi_length=length(xi_al)-1;
    for i=1:xi_length
        % ideal path traveled (based on interpolated values)
        sm.sub{partNo}.session{sesNo}.trial{k}.ideal_path_length_interpol=sm.sub{partNo}.session{sesNo}.trial{k}.ideal_path_length_interpol+sum(sm_distance(xi_al(i),xi_al(i+1),yi_al(i),yi_al(i+1)));
    end
...
%     hold off
    ...
    if sm.sub{partNo}.session{sesNo}.trial{k}.fb==0
