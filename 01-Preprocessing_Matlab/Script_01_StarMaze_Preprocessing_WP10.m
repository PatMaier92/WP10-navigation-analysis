...
%                 xlim([0 1]); ylim([0 1]); hold off; 
                ...
                [sm.participant(p).session(s).trial(k).correct_goal, sm.participant(p).session(s).trial(k).correct_goal_ego,...
                    sm.participant(p).session(s).trial(k).correct_final_alley, sm.participant(p).session(s).trial(k).correct_final_alley_ego]=computeSuccess(...
                    criterion, sm.participant(p).session(s).trial(k).final_distance, sm.participant(p).session(s).trial(k).final_distance_ego,...
                    sm.participant(p).session(s).trial(k).goal_alley, sm.participant(p).session(s).trial(k).ego_alley, ...
...
%                     output_folder)
                ...
                %% create plot 
                plotMotorControlTrial(sm.participant(p).session(s).trial(k).trial_num, sm.participant(p).session(s).session_number,...
