...
%                 xlim([0 1]); ylim([0 1]); hold off; 
                ...
                % SEARCH STRATEGY in ALLOCENTRIC probe trials with potential EGOCENTRIC response
                sm.participant(p).session(s).trial(k).search_strategy_in_allo="tbd";
%                 if sm.participant(p).session(s).trial(k).condition=="allo_ret" && mod(sm.participant(p).session(s).trial(k).start_i,2)
...
%                 end
                ...
                plotTrialTrack(sm.participant(p).session(s).trial(k).trial, sm.participant(p).session(s).session_num,...
                    ...
                    sm.participant(p).session(s).trial(k).search_strategy, sm.participant(p).session(s).trial(k).search_strategy_in_allo,...
