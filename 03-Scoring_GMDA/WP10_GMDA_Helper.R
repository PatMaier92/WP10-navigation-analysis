...
data_gmda <- gmda_data %>% 
  select(!c(`Measure Type`, Rotation)) %>% 
  filter(!Measure %in% c("Canonical Organization", "Rotational Bias", "Scaling Bias", "Num Landmarks Missing")) %>% 
...
out_file_R <-  paste("../WP10_data/WP10_results/wp10_GMDA_data_", date, ".Rdata", sep="")
