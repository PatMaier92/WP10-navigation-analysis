...
probe.time_t <- lme(1/time ~ group*session*condition,
                    random=~session+condition | id,
                    data=data, method="ML")
...
data_ego <- data %>% 
  ...
  select(id, group, session, trial_num, block, condition, goal_i, correct_final_alley) %>% 
...
t1 <- data_l %>% 
  select(id, group, session, trial_num, goal_i, correct_final_alley, time, path_distance) %>% 
  mutate(goal_f=factor(goal_i)) %>% 
...
t2 <- data %>% 
  select(id, group, session, condition, trial_num, goal_i, correct_final_alley, final_distance, time, path_distance) %>% 
  mutate(goal_f=factor(goal_i)) %>% 
  filter(session==1) %>% 
