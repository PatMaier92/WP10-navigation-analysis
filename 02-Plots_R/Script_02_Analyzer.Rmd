...
anova.lme(learn.time_base, learn.time_var)

plot(learn.time_var, resid(., type="p") ~ fitted(.) | group, abline=0)
plot(learn.time_var, group ~ resid(., type="p"))
plot(learn.time_var, block_f ~ resid(., type="p"))
qqnorm(resid(learn.time_var))
qqline(resid(learn.time_var))

# TBD: refit with REML

# main effects & interactions  
anova.lme(learn.time_var, type="marginal", adjustSigma=T)

# post-hoc comparisons 
emtrends(learn.time_var, pairwise ~ block_f, var="trial_in_cond_c", adjust="bonferroni")
emtrends(learn.time_var, pairwise ~ group, var="trial_in_cond_c", adjust="bonferroni")
emmeans(learn.time_var, pairwise ~ group, type="response", adjust="bonferroni")


# ## optional: advanced lme model withother types of variance estimation
...
# contrasts set in beginning, check options("contrasts") if in doubt 
learn.path_var <- update(learn.path_base, weights=varIdent(form=~1 | group))
anova(learn.path_base, learn.path_var) 

plot(learn.path_var, resid(., type="p") ~ fitted(.) | group)
plot(learn.path_var, group ~ resid(., type="p"))
qqnorm(resid(learn.path_var))
qqline(resid(learn.path_var))
...
## 1) standard lme model without variance estimation 
# contrasts set in beginning, check options("contrasts") if in doubt 
learn.edit_base <- lme(zone_editdistance ~ group*block_f*trial_in_cond_c,
                       random=~1+block_f | id, 
                       data=data_l, method="ML")

## 2) advanced lme model wit variance estimation
# contrasts set in beginning, check options("contrasts") if in doubt 
learn.edit_var <- update(learn.edit_base, weights=varIdent(form=~1 | group))
anova(learn.edit_base, learn.edit_var) 

plot(learn.edit_var, resid(., type="p") ~ fitted(.) | group)
plot(learn.edit_var, group ~ resid(., type="p"))
qqnorm(resid(learn.edit_var))
qqline(resid(learn.edit_var))
...
## 2) advanced lme model wit variance estimation
# contrasts set in beginning, check options("contrasts") if in doubt 
probe.fd_correct_var <- update(probe.fd_correct, weights=varIdent(form=~1 | group))
anova(probe.fd_correct, probe.fd_correct_var) 

# diagnostics
plot(probe.fd_correct_var, resid(., type="p") ~ fitted(.) | group)
plot(probe.fd_correct_var, group ~ resid(., type="p"))
qqnorm(resid(probe.fd_correct_var))
qqline(resid(probe.fd_correct_var))

# TBD: refit with REML

# statistics 
anova(probe.fd_correct_var, type="marginal", adjustSigma=T)
emmeans(probe.fd_correct_var, pairwise ~ group, adjust="bonferroni")
emmeans(probe.fd_correct_var, pairwise ~ session)
...
## 2) advanced lme model wit variance estimation
# contrasts set in beginning, check options("contrasts") if in doubt 
probe.fd_correct_var_1 <- update(probe.fd_correct_1, weights=varIdent(form=~1 | group))
anova(probe.fd_correct_1, probe.fd_correct_var_1) 

# diagnostics
plot(probe.fd_correct_var_1, resid(., type="p") ~ fitted(.) | group)
plot(probe.fd_correct_var_1, group ~ resid(., type="p"))
qqnorm(resid(probe.fd_correct_var_1))
qqline(resid(probe.fd_correct_var_1))

# statistics 
anova(probe.fd_correct_var_1, type="marginal", adjustSigma=T)
emmeans(probe.fd_correct_var, pairwise ~ group, adjust="bonferroni")
...
## 2) advanced lme model wit variance estimation
# contrasts set in beginning, check options("contrasts") if in doubt 
probe.fd_local_var <- update(probe.fd_local, weights=varIdent(form=~1 | group))
anova(probe.fd_local, probe.fd_local_var) 

# diagnostics
plot(probe.fd_local_var, resid(., type="p") ~ fitted(.) | group)
plot(probe.fd_local_var, group ~ resid(., type="p"))
qqnorm(resid(probe.fd_local_var))
qqline(resid(probe.fd_local_var))

# statistics 
anova(probe.fd_local_var, type="marginal", adjustSigma=T)
emmeans(probe.fd_local_var, pairwise ~ group, adjust="bonferroni")
emmeans(probe.fd_local_var, pairwise ~ session)
...
## 2) advanced lme model wit variance estimation
# contrasts set in beginning, check options("contrasts") if in doubt 
probe.fd_local_var_1 <- update(probe.fd_local_1, weights=varIdent(form=~1 | group))
anova(probe.fd_local_1, probe.fd_local_var_1) 

# diagnostics
plot(probe.fd_local_var_1, resid(., type="p") ~ fitted(.) | group)
plot(probe.fd_local_var_1, group ~ resid(., type="p"))
qqnorm(resid(probe.fd_local_var_1))
qqline(resid(probe.fd_local_var_1))

# statistics 
anova(probe.fd_local_var_1, type="marginal", adjustSigma=T)
emmeans(probe.fd_local_var_1, pairwise ~ group, adjust="bonferroni")
...
## 2) advanced lme model wit variance estimation
# contrasts set in beginning, check options("contrasts") if in doubt 
probe.time_var <- update(probe.time, weights=varIdent(form=~1 | group))
anova(probe.time, probe.time_var) 

# diagnostics
plot(probe.time_var, resid(., type="p") ~ fitted(.))
plot(probe.time_var, resid(., type="p") ~ fitted(.) | group)
plot(probe.time_var, group ~ resid(., type="p"))
qqnorm(resid(probe.time_var))
qqline(resid(probe.time_var))

# TBD: refit with REML

# statistics 
anova(probe.time_var, type="marginal", adjustSigma=T)
emmeans(probe.time_var, pairwise ~ group | condition | session, adjust="bonferroni")
...
## 2) advanced lme model wit variance estimation
# contrasts set in beginning, check options("contrasts") if in doubt 
probe.time_var_1 <- update(probe.time_1, weights=varIdent(form=~1 | group))
anova(probe.time_1, probe.time_var_1) 

# diagnostics
plot(probe.time_var_1, resid(., type="p") ~ fitted(.))
plot(probe.time_var_1, resid(., type="p") ~ fitted(.) | group)
plot(probe.time_var_1, group ~ resid(., type="p"))
qqnorm(resid(probe.time_var_1))
qqline(resid(probe.time_var_1))

# TBD: refit with REML

# statistics 
anova(probe.time_var_1, type="marginal", adjustSigma=T)
emmeans(probe.time_var, pairwise ~ group | condition, adjust="bonferroni")
...
# diagnostics
plot(probe.rot_var, resid(., type="p") ~ fitted(.))
plot(probe.rot_var, resid(., type="p") ~ fitted(.) | group)
plot(probe.rot_var, group ~ resid(., type="p"))
qqnorm(resid(probe.rot_var))
qqline(resid(probe.rot_var))
