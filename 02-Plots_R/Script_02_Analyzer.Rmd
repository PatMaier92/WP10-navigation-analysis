...
temp <- data %>% filter(condition=="allo_ret")
table(temp$correct_final_alley_ego, temp$group)
# TBD: this is so rare that it even seems below chance level, is this even meaningful?  
...
probe.fd_ego <- lme(final_distance_ego ~ group*session,
                    random=~1+session | id, 
                    data=data %>% filter(condition=="allo_ret"), na.action=na.omit, method="ML")
...
# helper plots 
ggplot(data %>% filter(condition=="allo_ret"), aes(x=final_distance_ego)) + geom_histogram()
ggplot(data %>% filter(condition=="allo_ret"), aes(x=group, y=final_distance_ego)) + geom_boxplot() + facet_grid(~ condition + session)
...
probe.path_ego <- lme(ego_path_distance ~ group*session,
                      ...
                      data=data %>% filter(condition=="allo_ret"), method="ML", na.action=na.omit)
...
# helper plots
ggplot(data %>% filter(condition=="allo_ret"), aes(x=ego_path_distance)) + geom_histogram()
ggplot(data %>% filter(condition=="allo_ret"), aes(x=group, y=ego_path_distance)) + geom_boxplot() + facet_wrap(~session)
...
# chose model 2 (only because model 3 cannot be re-fit with REML without convergence issues)
...
# helper plots
ggplot(data %>% filter(condition=="allo_ret"), aes(x=ego_target_distance_error)) + geom_histogram()
ggplot(data %>% filter(condition=="allo_ret"), aes(x=group, y=target_distance_error)) + geom_boxplot() + facet_wrap(~session)
