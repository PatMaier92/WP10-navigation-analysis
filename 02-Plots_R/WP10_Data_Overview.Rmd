...
* **Path score** (metric): TBD. Number of zones entered. 
* **Path length** (metric): Absolute length of traveled path to chosen goal location. Calculated as summation of distance between x-/y-coordinates over time. Value can be misleading if chosen location is not correct location (e.g., shorter path if incorrect but closer location selected). 
$$P = \sum \sqrt{(x_{t}-x_{t+1})^2 + (y_{t}-y_{t+1})^2}$$
* **Avg. distance to ideal path to goal** (metric): Average cumulative distance between chosen path and (interpolated) ideal path to the correct goal. Calculated as summation of distance between x-/y-coordinates of chosen and (interpolated) ideal path, then divided by the number of x-/y data points. Marker of how closely the participant stuck to the ideal path to the correct goal. Value can be misleading if chosen location is not correct goal location.
$$DPG = \frac {\sum \sqrt{(x_{t}-x_{interpolated-ideal-path})^2 + (y_{t}-y_{interpolated-ideal-path})^2}} {n_{t}}$$
* **Adjusted avg. distance to ideal path to goal** (metric): Same as above but the chosen path is reduced to unique data points i.e., rotation on the spot/waiting at a spot is removed from the calculation. 
* **(Adjusted) avg. distance to ideal path to *chosen* goal** (metric): Same as above but does not consider ideal path to the correct goal but to the chosen goal. Marker of how efficiently the participant navigated to the chosen goal, therefore more comparable across correct/incorrect trials.
* **Time**: Time to complete trial in seconds. Can also be computed for zones separately (e.g., time spend in relevant/irrelevant areas).
* **Sum of head rotation**: TBD. Sum of z-rotations indicating exploration/looking around. Can also be computed for zones separately (e.g., initial orientation phase).
* **Strategy classification**: Direct to chosen goal, detour (entering more zones than necessary), reorientation (re-entering same zones). 
...
How well did participants learn the position of the three goal locations at Day 1?  
