...
* Consolidated retrieval in egocentric and allocentric navigation condition (5 trials per goal location; 15 in total).
...
* Day 1: 5 min for practise, 22 min for learning = 27 total
* Day 13: 7 min for retrieval, 3 min for non-navigational tests = 10 total

*Older kids*: 

* Day 1: `r mean(5, 6, 5, 6, 4, 5, 6, 5)` min for practise (range: `r range(5, 6, 5, 6, 4, 5, 6, 5)`), `r mean(18, 15, 17, 29, 18, 21, 25, 22)` min for learning (range: `r range(18, 15, 17, 29, 18, 21, 25, 22)`) = `r mean(18, 15, 17, 29, 18, 21, 25, 22) + mean(5, 6, 5, 6, 4, 5, 6, 5)` total (range: `r range(18+5, 15+6, 17+5, 29+6, 18+4, 21+5, 25+6, 22+5)`)
* Day 13: `r mean(7, 5, 5, 4, 4, 5, 8, 7, 8)` min for retrieval (range: `r range(7, 5, 5, 4, 4, 5, 8, 7, 8)`), 3 min for non-navigational tests = `r mean(7, 5, 5, 4, 4, 5, 8, 7, 8) +  3` total (range: `r range(7+3, 5+3, 5+3, 4+3, 4+3, 5+3, 8+3, 7+3, 8+3)`)


### Variables of interest

In learning trials, the goal is visible, i.e. the trial is always completed successfully except if the participants needs more than 120 seconds (time out). In egocentric and allocentric retrieval trials, the goal is not visible. Participants go to the place where they remember the goal being located and press a button. 
...
plot_grid(pt1, pt2, pt3, pt4, ncol=1, labels="AUTO", rel_heights=c(1.1, 0.93, 0.93, 0.93))
...
*First plot:* As expected, the preliminary results show age-related performance differences, i.e. a better performance with higher age. So far, we have not collected any data from older adults, so we do not know if the effect reverses with older age. The age-dependent performance trajectory is present in all conditions, i.e. in both allocentric and egocentric trials and during immediate retrieval at Day 1 as well as consolidated retrieval at Day 13. Notably, performance variability seems to be higher in old kids compared to young adults, i.e. some old kids perform like adults whereas others struggle much more. We could probably take a closer look at the reasons behind this variability with the structural MR data in the B04 project in Berlin (start in 2021). When looking at the difference between Day 13 and Day 1 (forgetting rate) older kids tend to have a lower success rate compared to younger adults, but a comparable decline in remembering the direct path. 

*Second plot:* When comparing allocentric vs. egocentric directly, the performance level seem  similar, indicating a relatively comparable task difficulty. Only for the ability to remember the direct path, both old kids and younger adults show higher performance in egocentric compared to allocentric condition.

``` {r, data_func_agg, include=FALSE}
```

``` {r, aggregate_bars, echo=FALSE, warning=FALSE, fig.asp=1.5, fig.align = "center"}

plot_grid(pagg1, paggd1, pagg2, paggd2, pagg3, paggd3, pagg4, paggd4, ncol=2, rel_widths=c(1.75,1.25), rel_heights=c(1.1, 0.93, 0.93, 0.93))

```

``` {r, aggregate_bars_2, echo=FALSE, warning=FALSE, fig.width=5, fig.asp=2, fig.align = "center"}

plot_grid(pagg5, pagg6, pagg7, pagg8, ncol=1, labels="AUTO", rel_heights=c(1.1, 0.93, 0.93, 0.93))

```

### Strategy use data 

We categorized participant's trajectories into different strategies (still under development). At the moment, strategies are a qualitative description of the trajectory and independent of success. For example, direct strategy means that the participant went straight to the chosen goal location without any detours or returns, independent of whether that chosen goal location was correct or incorrect. 
...
table_dat <- sm_trial_data %>% 
  ...
  summarise(path=mean(path_abs[success==1], na.rm = TRUE),
...
``` {r, table_learn, echo=FALSE, message=FALSE, results='asis' }
...
``` {r, table_alloego, echo=FALSE, message=FALSE, results='asis' }
