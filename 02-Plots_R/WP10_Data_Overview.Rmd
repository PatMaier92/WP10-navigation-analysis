...
``` {r, data_func_scatter, include=FALSE}
```

``` {r, scatter, echo=FALSE, warning=FALSE, fig.width=7, fig.height=4, fig.align="center"}

sc1 + 
  plot_annotation(title="Relation between egocentric and allocentric performance",
                  tag_levels = 'A')

```
# ``` {r, data_func_rotation, include=FALSE}
# ```
# 
# ``` {r, rotation, echo=FALSE, warning=FALSE, fig.width=8, fig.height=10, fig.align="center"}
# 
# rot1 + guide_area() + rot2 + rot3 + rot4 + rot5 + 
#   plot_annotation(title= "Initial 'head' rotation", tag_levels = 'A') +
#   plot_layout(ncol=2, guides="collect") 
# 
# ```
...
```{r, load_post_data, include=FALSE}
in_file <- "../WP10_data/WP10_results/WP10_post_results_table.RData"
load(in_file)
rm(in_file)
```

``` {r, data_func_post_tests, include=FALSE}
```

``` {r, post_tests, echo=FALSE, warning=FALSE, fig.width=10, fig.height=7, fig.align="center" }

nonav + plot_spacer() + shape + landmark + 
  plot_annotation(title="Performance in non-navigational memory tasks",
                  tag_levels = 'A') +
  plot_layout(height=c(1.2,1.8), guides='auto')
