...
* no other interactions, especially not *group x session* interaction
* *group* main effect: younger participants had worse memory accuracy than older participants 
...
(ref:p-ms-caption2) Post-hoc contrasts for group x condition interaction.
...
(ref:p-ms-caption3) Post-hoc contrasts for factor group.
...
```{r, include=F}
rm(probe.memory_s, R2, apa_lmm, apa_emm1, emm1)
```
...
emm1 <- test(emtrends(probe.time_s, ~ group, var="sessionC", lmer.df="satterthwaite"), adjust="none")
# TBD adjust for 6 comparisons

emm2 <- emtrends(probe.time_s, pairwise ~ group, var="sessionC", lmer.df="satterthwaite", adjust="none")
# TBD adjust for 6 comparisons

emm3 <- emmeans(probe.time_s, ~ group * condition, lmer.df="satterthwaite")
con3 <- contrast(emm3, con_list_group_condition, adjust="bonferroni")
apa_emm3 <- apa_print(con3)
apa_emm3 <- apa_emm3$table %>% remove_rownames()

emm4 <- test(emtrends(probe.time_s, ~ condition, var="sessionC", lmer.df="satterthwaite"), adjust="none")
# TBD adjust for 3 comparisons

emm5 <- emtrends(probe.time_s, pairwise ~ condition, var="sessionC", lmer.df="satterthwaite", adjust="none")
# TBD adjust for 3 comparisons
```

The model had a fit of $R2 = `r R2$Rsq`, [CI: `r R2$upper.CL`, `r R2$lower.CL`$].

* *group x session* interaction: the slopes in all groups did not differ from zero, the slope of 6-7-year-old children was significantly steeper than the slope of adults. 
...
* *session x condition* interaction: the slopes in both conditions did not differ from zero, the slopes across sessions differed between egocentric and allocentric.
...
Post-hoc contrasts for group * session interaction.
...
Post-hoc contrasts for condition * session interaction.

```{r table-probe-time5, echo=F}
emm4
```

```{r table-probe-time6, echo=F}
emm5
```

```{r, include=F}
rm(probe.time_s, apa_lmm, apa_emm3, emm1, emm2, emm3, emm4, emm5)
```
...
```{r, include=F}
rm(probe.excess_path_s, apa_lmm, apa_emm2)
```
...
```{r, include=F}
rm(probe.rotation_path_s, apa_lmm, apa_emm2)
```
