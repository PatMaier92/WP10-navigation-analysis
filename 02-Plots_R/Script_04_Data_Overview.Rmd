...
line_t + line_p + line_d + line_ir + line_rp +
...
**In allocentric probe trials: Did participants enter the egocentric area** (only trials with outer starts)

```{r, warning=F, echo=F }

box_allo_cov_ego + box_allo_time_ego + 
  plot_annotation("Egocentric navigation behavior in allocentric probe trials") + 
  plot_layout(guides="collect") & theme(legend.position="bottom", legend.justification=c(0,0))

rm(box_allo_cov_ego, box_allo_time_ego)
```
```{r, stats_probe_cov_ego_in_allo}
```

\newpage

**In allocentric probe trials: Did participants enter the original start area (homing)**

```{r, warning=F, echo=F }

box_allo_cov_start + box_allo_time_start + 
  plot_annotation("Homing navigation behavior in allocentric probe trials") + 
  plot_layout(guides="collect") & theme(legend.position="bottom", legend.justification=c(0,0))

rm(box_allo_cov_start, box_allo_time_start)
```

```{r, stats_probe_cov_start_in_allo}
```
