...
* Navigation to and remembering of three goal locations. Baseline retrieval in egocentric (observer-dependent, i.e. from same starting point but without landmark cues) and allocentric (observer-independent, i.e. from new starting points) (15 trials * 3 goal locations = 45 trials).

*Day 14*

* Retrieval in egocentric and allocentric condition (5 trials * 3 locations = 15 trials).
* Post-navigation memory test: Recognize maze shape, landmarks and goal objects, position landmarks and goal object (4 trials).
...
* **Correct goal alley** (yes/no): Finding the correct goal alley, defined as the correct outer arm or intersection right in front. 
* **Final distance & memory score** (metric): Final distance between x-/y-coordinates of the chosen and the correct goal location. Memory score normalizes the final distance value as a score from 0 to 1 (chance level 0.5) by comparing final distance to the final distance of 1000 randomly sampled and uniformly distributed locations in the maze.  
...
* **Excess path length** (metric): The travelled path length minus the ideal path length to the chosen location. The path lengths are calculated as summation of distance between x-/y-coordinates over time.
$$\text{(Ideal) path length} = \sum \sqrt{(x_{t}-x_{t+1})^2 + (y_{t}-y_{t+1})^2}$$
$$\text{Excess path length} = (path-path_{ideal})$$
* **Presence in zone** (metric): Proportion of time/data points spend in a defined area.
$$\text{Presence in zone} = \frac {n(xy_{in zone})} {n(xy)} $$
* **Rotation**: The sum of absolute z-rotations over time divided by 360° (i.e. full turns). Takes into account the switch at 0/360°. Variants are 1. rotation in the initial area, 2. rotation normalized by path length (because longer paths automatically involve more rotation in a Starmaze environment). 
...
![Visualization for variables](./pics/Var_all.jpg){width=300px,height=500x}
...
line_t + line_p + line_pa + line_ir + line_rp +
...
rm(line_t, line_p, line_pa, line_ir, line_rp, sm_trialwise)
```
...
box_ego_pa + labs(subtitle="Egocentric") + box_allo_pa + labs(subtitle="Allocentric") +
  plot_annotation("Presence in outer alleys (vs. inner pentaton)") + 
  plot_layout(guides="collect") & theme(legend.position="bottom", legend.justification=c(0,0))

rm(box_ego_pa, box_ego_pa)
```

```{r, stats_probe_presence}
```
...
**Memory score (correct trials) for spatial precision**

```{r, warning=F, echo=F, fig.height=4 }

box_ego_cor_ms + labs(subtitle="Egocentric") + box_allo_cor_ms + labs(subtitle="Allocentric") +
  plot_annotation("Memory score in all correct probe trials for spatial precision") + 
  plot_layout(guides="collect") & theme(legend.position="bottom", legend.justification=c(0,0))

rm(box_ego_cor_ms, box_allo_cor_ms)
```

```{r, stats_probe_ms_in_corr, fig.height=4 }
```

```{r, warning=F, echo=F }

box_ego_delta_cor_ms + labs(subtitle="Egocentric") + box_allo_delta_cor_ms + labs(subtitle="Allocentric")  +
  plot_annotation("Change in memory score in correct probe trials") + 
  plot_layout(guides="collect") & theme(legend.position="bottom", legend.justification=c(0,0))

rm(box_ego_delta_cor_ms, box_allo_delta_cor_ms)
```

```{r, stats_probe_ms_corr_change }
```

\newpage

### Exploration allocentric trials 

#### Navigation behavior

**How present were participants in specific zones that indicate an egocentric or homing strategy** (only trials where egocentric alley is not original start alley, egocentric response only for trials with defined egocentric location)

```{r, warning=F, echo=F, fig.height=4 }

box_allo_pres +  
  plot_annotation("Exploration of presence in allocentric probe trials") & theme(legend.position="bottom", legend.justification=c(0,0))

rm(box_allo_pres)
```

```{r, stats_probe_presence_in_allo}
```
...
**How often did participants erroneously pick locations that indicate an egocentric or homing strategy** (only trials where egocentric alley is not original start alley, egocentric response only for trials with defined egocentric location)

```{r, warning=F, echo=F, fig.height=4 }

box_allo_msa +  
  plot_annotation("Exploration of memory score in allocentric probe trials") & theme(legend.position="bottom", legend.justification=c(0,0))

rm(box_allo_msa)
```

```{r, stats_probe_memory_in_allo }
```
...
Strategies are a qualitative description of the trajectory, independent of accuracy. For example, direct strategy means that the participant went straight to the chosen goal location without any detours or reorienting, independent of whether the chosen goal location was correct or incorrect. 
...
```{r, warning=F, echo=F, fig.height=4, fig.width=4 }
