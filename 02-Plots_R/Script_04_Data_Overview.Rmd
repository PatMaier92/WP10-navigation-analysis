...
dots_ego <- dot_plots(data_p %>% filter(condition=="ego_ret"), "x_n", "y_n", "goal_f", 
                      "goal_x", "goal_y", "Egocentric trials", mylabels)

dots_allo <- dot_plots(data_p %>% filter(condition=="allo_ret"), "x_n", "y_n", "goal_f", 
                       "goal_x", "goal_y", "Allocentric trials", mylabels)
...
TBD r, stats_probe_ms_change_AOV 

TBD r, stats_probe_ms_change_WRS2
...
```{r, echo=F, fig.height=5, fig.width=5, fig.cap="Change in spatial precision in correct probe trials." }
...
TBD r, stats_probe_ms_change_corr_AOV 

TBD r, stats_probe_ms_change_corr_WRS2 
...
layout_avg <- ggplot(layout_data_avg, aes(x=group, y=perc, fill=group, color=group)) + 
  geom_bar(stat="identity", position=position_dodge()) +
  scale_x_discrete(labels=group_labels) + 
...
rm(layout_data, layout_data_avg)
```

```{r, plots_landmarks, include=F }
...
# averaged
landmark_avg <- ggplot(pt_data %>% filter(condition=="landmarks"), 
                       aes(x=group, y=score, color=group, shape=group)) + 
...
rm(landmark_comp_data)
```

```{r, plots_positioning, include=F }

# averaged
positioning_avg <- ggplot(pt_data %>% filter(condition=="position"), 
                       aes(x=group, y=score, color=group, shape=group)) + 
  stat_summary(fun.data=mean_cl_normal, na.rm=TRUE, geom="errorbar", width=0.2, size=1.5) +
  stat_summary(fun=mean, na.rm=TRUE, geom="point", size=3) +
  scale_color_manual(values=group_colors, labels=group_labels, name=NULL) +
  ...
  theme(legend.position="top", legend.justification=c(0,0),
        ...
        axis.ticks.x=element_blank(),
        axis.text.x=element_blank()) +
  labs(x=NULL, y="gmda positioning score", 
       caption="mean and 95% CI")
```

```{r, echo=F, fig.height=4, fig.width=8, fig.cap="Scores in non-navigational memory tests." }

layout_avg + 
  (landmark_avg + guides(color="none", shape="none")) + 
  (positioning_avg + guides(color="none", shape="none")) + 
  plot_annotation(title="Non-navigational memory tests") +
  plot_layout(guides="collect") & theme(legend.position="bottom", legend.justification=c(0,0))

rm(layout_detail, layout_avg, landmark_comparison, landmark_avg, positioning_avg)
```
...
No significant group differences in landmark recognition. 
