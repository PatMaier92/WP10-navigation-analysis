...
* **Avg. target proximity** (metric): This measure reflects if participants tended to stay closer or further away from the correct goal location (TBD REFERENCES). It is calculated as the cumulative Euclidean distance between the the player’s x-/y-coordinates along their path and the correct goal location, averaged by the number of x-/y data points. 
– TBD Variation: Deviation to "ideal" value
$$\text{Avg. target proximity} = \frac {\sum_{i=1}^{n}\sqrt{(x_{i}-x_{correct})^2 + (y_{i}-y_{correct})^2}} {n}$$
...
### **Average target proximity**

```{r, model_probe_proximity }
```

```{r, plot_probe_proximity, include=F }
```
...
```{r, include=F}
rm(model.proximity, apa_lmm, apa_emm1, apa_emm2, apa_emm3)
```
...
```{r, model_learn_proximity }
```

```{r, plot_learn_proximity, include=F}
```

```{r, echo=F, fig.height=4, fig.width=5, fig.cap="Trial curve for excess path in training." }
plot.proximity_learn
rm(plot.proximity_learn)
```

TBD

```{r, include=F}
rm(model.proximity_learn, R2, apa_lmm, apa_emm, emm)
```
