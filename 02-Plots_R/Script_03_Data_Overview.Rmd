...
$$\text{Memory score} = \text{percentile rank of the Euclidean distance value in relation the the distribution of Euclidean distance values from 1000 random locations}$$
...
* **Excess avg. target distance** (metric): This measure reflects if participants tended to stay closer or further away from the correct goal location (TBD REFERENCES). It is calculated as the avg. target distance minus the ideal avg. target distance to the goal location. The avg. target distance is calculated as the cumulative Euclidean distance between the the player’s x-/y-coordinates along their path and the correct goal location, averaged by the number of x-/y data points. 
$$\text{Avg. target distance} = \frac {\sum_{i=1}^{n}\sqrt{(x_{i}-x_{correct})^2 + (y_{i}-y_{correct})^2}} {n}$$
$$\text{Excess avg. target distance} = (distance-distance_{ideal})$$
* **Angular velocity** (metric): **TBD: IdPhi, VTE-behavior (REFERENCES)**. z-rotations from the initial area (start alley plus first intersection) were transformed from degrees to radians and unwrapped (i.e. discontinuities removed due to switch at 360°). We then calculated the sum of absolute z-rotations in the initial area (via the first derivative), divided by the number of data points to obtain the averaged angular velocity. (REFERENCES)
$$\text{Angular Velocity (radians)} = \frac {\sum_{i=1}^{n_{initial}} | (z_{i}-z_{i+1})|} {n_{initial}}$$
* **Initial and total rotation**: **meaning (REFERENCES)**. z-rotations were transformed from degrees to radians, unwrapped (i.e. discontinuities removed due to switch at 360°), and back-transformed to degrees. We then calculated the sum of absolute z-rotations over time divided by 360° for full turns. Variants of this measure are 1. Rotation in the initial area (start alley plus first intersection), 2. Rotation normalized by path length (because longer paths automatically involve more rotation in the Starmaze).
$$\text{Initial Rotation (degree)} = \frac {\sum_{i=1}^{n_{initial}} | (z_{i}-z_{i+1}) |} {360}$$ 
$$\text{Total rotation by path length (degree)} = \frac {\sum_{i=1}^{n} | (z_{i}-z_{i+1})|} {path} * \frac {1} {360}$$ 
...
### **Excess average target distance**
...
con1 <- emmeans(model.target_distance, pairwise ~ group, lmer.df="satterthwaite", adjust="satterthwaite")$contrasts
apa_emm1 <- apa_print(con1)
apa_emm1 <- apa_emm1$table %>% remove_rownames()
rm(con1)
...
(ref:p-ed-caption0) Fixed effects for excess target distance in probe trials.
...
### **Angular velocity (IdPhi)**

```{r, model_probe_angular_velocity }
```

```{r, plot_probe_angular_velocity, include=F }
```
...
```{r, include=F}
rm(model.angular_velocity, apa_lmm, apa_emm2, apa_emm3)
```
...
apa_lmm <- apa_print(model.initial_rotation)
apa_lmm <- apa_lmm$table %>% mutate(term=str_replace_all(term, c(" c| f"), ''))
...
(ref:p-ir-caption1) Post-hoc contrasts for group x session interaction.
...
(ref:p-ir-caption2) Post-hoc contrasts for group x condition interaction.
...
(ref:p-ir-caption3) Post-hoc contrasts for session x condition interaction.

```{r table-probe-init-rot3, echo=F}
apa_table(
  apa_emm3,
  caption="(ref:p-ir-caption3)",
  placement="h")
```

```{r, include=F}
rm(model.initial_rotation, apa_lmm, apa_emm1, apa_emm2, apa_emm3)
```
...
```{r, include=F}
rm(model.rotation_path, apa_lmm, apa_emm1, apa_emm2, apa_emm3)
```
...
The excess avg. target distance decreased across trials (main effect *trial in block*) but there were no group differences in the slope of the decrease (no interaction *group x trial in block* ). The groups differed in excess avg. target distance length (main effect *group*). More specifically 6-7 year-old children had higher excess avg. distance to target than the other two groups, meaning they navigated further away from the correct goal location during the course of the trial.
(Table\ \@ref(tab:table-learn-excess-distance0))

(ref:l-ed-caption0) Fixed effects for excess avg. target distance in training.
...
```{r, model_learn_angular_velocity }
```

```{r, plot_learn_angular_velocity, include=F}
```

```{r, echo=F, fig.height=4, fig.width=5, fig.cap="Trial curve for excess path in training." }
plot.angular_velocity_learn
rm(plot.angular_velocity_learn)
```

TBD

```{r, include=F}
rm(model.angular_velocity_learn, R2, apa_lmm, apa_emm, emm)
```
...
rm(plot.initial_rotation_learn)
```
...
apa_lmm <- apa_print(model.initial_rotation_learn)
apa_lmm <- apa_lmm$table %>% mutate(term=str_replace_all(term, c(" c| f"), ''))
...
(ref:l-ir-caption0)  Fixed effects for factor initial rotation.
...
```{r, include=F}
rm(model.initial_rotation_learn, R2, apa_lmm, apa_emm, emm)
```
