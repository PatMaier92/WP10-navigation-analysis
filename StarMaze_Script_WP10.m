...
% for trial-info trial_results.csv required

% Script starts with input: 
%   1. in case u add an analysis, change p for loop p is the next bumber in
%   the starmaze-table
...
% Check functions, whether you have to adjust for alterations of the star
% maze or whether your folders are called correctly
%You can quit by entering CTRL+C
...
% Block 1: 
% In the following block "test-figure" the script creates a starmaze depending on the
% provided coordinates of the corners of the starmaze. The coordinates are
% taken from a provided .csv.

% Block 2: 
% In the next block, the script will write you csv-data to xlsx-file, if you have
% xls/xlsx-data already, you can skip that step
% The script assumes you have a log-file and a file called "trial_results"
...
%% Data-collection sanity check & Data preparation
...
for k=1:numel(files)
name = files{k}; 
    % jump unnecessary files (log & trial_results)
        jump1 = strfind(name,'trial_results');
        jump2 = strfind(name,'log');
...
[sm.sub{p}.session{s}.trial{k}.goal_x_ego, sm.sub{p}.session{s}.trial{k}.goal_y_ego, x_line, y_line,x_line_ego, y_line_ego,...
    ...
    sm.sub{p}.session{s}.trial{k}.ideal_time_ego,sm.sub{p}.session{s}.trial{k}.ideal_headturnNo,sm.sub{p}.session{s}.trial{k}.ideal_velocity]=sm_wp10_depStartp_Variables2(sm.sub{p}.session{s}.trial{k}.start,...
...
% Path-Analysis  
    sm.sub{p}.session{s}.trial{k}.result.distance_traveled=0;dist_to_goal=0; % start-initiation
...
% determine final position in outer area of alley
...
% determine direct path to target
...
%% writing data ---> result sheet XLSX for single trials %%%%
% header
...
col_header_4={'exploration','success','success_ego','correct_final_alley','final_deviation',...
 ...
 % name of excel-file  
        ...
        file_name = strrep(['results_' num2str(wp) '_' sm.sub{p}.Group '_' ID '_' Session '_' Trial],'csv','xls');
        new_file = fullfile(folderOut, file_name);
% write data     
     xlswrite(new_file,[group_var sm.sub{p}.session{s}.trial{k}.result.time sm.sub{p}.session{s}.trial{k}.result.time_accuracy sm.sub{p}.session{s}.trial{k}.result.time_accuracy_ego...
      ...
      xlswrite(new_file,[group_var sm.sub{p}.session{s}.trial{k}.zone.alley_zone(1,1) sm.sub{p}.session{s}.trial{k}.zone.alley_zone(1,2) sm.sub{p}.session{s}.trial{k}.zone.alley_zone(1,3) sm.sub{p}.session{s}.trial{k}.zone.alley_zone(1,4) sm.sub{p}.session{s}.trial{k}.zone.alley_zone(1,5)...
     ...
     xlswrite(new_file,[group_var sm.sub{p}.session{s}.trial{k}.result.exploration sm.sub{p}.session{s}.trial{k}.result.success sm.sub{p}.session{s}.trial{k}.result.success_ego sm.sub{p}.session{s}.trial{k}.result.correct_final_alley sm.sub{p}.session{s}.trial{k}.result.final_deviation ...
...
new_name2 = strrep([sm.sub{p}.Group '_' num2str(sm.sub{p}.id)  '_results'],'xlsx','xlsx');
